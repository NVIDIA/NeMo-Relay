...
def native_entrypoint_source(platform: Platform) -> bytes:
    """Return the JavaScript entry point that loads one native binary."""
    return (
        "// Generated by scripts/package-node-bin.py.\n"
        "// Runtimes that do not resolve a `.node` file as a package entry point, such as\n"
        "// the Bun runtime, need this indirection to load the package by name.\n"
        f"module.exports = require('./{platform.binary}')\n"
    ).encode()
...
def build_native_package(node_dir: Path, platform: Platform, version: str, output: Path) -> Path:
    ...
    manifest = {
        ...
        **repository_metadata(source_manifest, include_engines=False),
        "main": NATIVE_ENTRYPOINT,
        "files": [NATIVE_ENTRYPOINT, platform.binary],
        "os": [platform.npm_os],
    ...
    with tarfile.open(destination, "w:gz") as archive:
        ...
        add_tar_bytes(archive, f"package/{NATIVE_ENTRYPOINT}", native_entrypoint_source(platform))
