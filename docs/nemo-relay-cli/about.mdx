...
| pi | Partial | Partial | No | Proof of concept. Tool and turn activity are captured through a Relay-authored pi extension, and tool calls can be blocked. pi's model traffic does not traverse the gateway, so there are no LLM spans and no model-call enforcement. |
