...
Hermes shell hooks and the `nemo-relay` gateway. This wrapped gateway path is
the release-safe Hermes integration path today. It is separate from the Hermes
third-party patch set under `patches/hermes-agent/`, which remains useful for
comparison and future plugin compatibility work but is not the priority path
for current Hermes validation.
...
`nemo-relay run -- hermes`. The wrapper starts a gateway on a dynamic
...
Dry-run stays inspection-only in this mode: it prints a placeholder ephemeral
gateway URL and does not bind a listener or launch Hermes.
