...
After NeMo Relay config exists, this shortcut is equivalent to
...
stores the skill name in `data.skill_name` and the source and tool name in
metadata. It does not retain skill paths.
