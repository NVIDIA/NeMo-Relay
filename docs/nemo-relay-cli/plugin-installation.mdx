...
Codex built-in `openai` provider to use the local Relay sidecar. The installed
base URL contains no credential. A managed assignment in `$CODEX_HOME/.env`
provides the per-user proof through Codex's `OpenAI-Project` header after restart.
Relay validates and removes that proof before observability and forwarding,
preserving any original project value. Installation and removal preserve other
`.env` settings. Restart Codex after either operation; no special launcher is
required. Legacy capability URLs remain supported for older installations. The
installer retains a `nemo-relay-openai` provider alias
