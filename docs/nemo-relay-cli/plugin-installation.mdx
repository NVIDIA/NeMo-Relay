...
Relay without a `nemo-relay` wrapper command. Claude Code and Codex use their
normal plugin systems; pi has no plugin system, so Relay writes an extension into
pi's own auto-discovery directory instead. See [pi](/nemo-relay-cli/pi).
