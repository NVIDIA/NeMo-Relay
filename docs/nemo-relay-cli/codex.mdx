...
Generated Codex hooks include `SessionStart`, `SubagentStart`, `SubagentStop`,
`PreToolUse`, `PostToolUse`, `PermissionRequest`, `PreCompact`, and `PostCompact`
for scope, tool, and mark events. Complete standalone `cat`,
`bat`, or PowerShell `Get-Content` reads of `SKILL.md` emit observed `skill.load`
marks. `UserPromptSubmit` and `Stop` are retained as private LLM correlation
hints and are not emitted as standalone NeMo Relay events.
