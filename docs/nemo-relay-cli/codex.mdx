...
Use persistent plugin installation when Codex should start normally and load
...
`nemo-relay-openai` provider alias at `http://127.0.0.1:47632`. It uses the
...
wrapper, user-level daemon, launchd agent, systemd user service, scheduled
