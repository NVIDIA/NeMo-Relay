...
`codex-cli >= 0.142.0`. This version provides the subagent hook lifecycle and
Responses client metadata that Relay uses for deterministic root, subagent,
tool, and LLM correlation. Relay retains compatibility fallbacks for older
payloads, but earlier Codex releases are outside the supported version floor.

<Warning>
Codex requires hooks to be manually reviewed and activated
...
Codex 0.142 delivers `SessionStart`, `SubagentStart`, `SubagentStop`,
`PreToolUse`, `PostToolUse`, `PermissionRequest`, `PreCompact`, `PostCompact`,
`UserPromptSubmit`, and `Stop`. Thread-spawned subagents share the root
`session_id`; their `agent_id` identifies the child scope. `UserPromptSubmit`
and `Stop` also provide private LLM correlation hints, and `Stop` creates the
turn boundary used for incremental ATIF snapshots.

The generated cross-host hook bundle also contains `SessionEnd`,
`PostToolUseFailure`, and `Notification` compatibility entries. Codex 0.142
ignores those unsupported events.
