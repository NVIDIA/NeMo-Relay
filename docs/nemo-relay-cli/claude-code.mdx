...
heartbeat defaults to 3 seconds and is shortened automatically when necessary
to remain below the gateway idle timeout. Set
...
verifies that its executable is named `nemo-relay`, and stops it. An open MCP
client can restart the managed gateway as part of its one allowed recovery.
