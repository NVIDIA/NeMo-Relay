...
The command authenticates the Relay process at the configured endpoint and
never stops a foreign process using the same endpoint. An open MCP client can
restart the managed gateway as part of its one allowed recovery.
...
gateway at the default daemon endpoint, `127.0.0.1:4040`. Pass the same
`--bind` value to both commands when using a custom endpoint.
