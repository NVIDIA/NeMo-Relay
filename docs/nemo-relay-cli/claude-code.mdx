...
Code, Codex, and Hermes MCP clients share a compatible gateway, and the gateway
...
waits for the MCP connection before session startup. The installed command
hook carries the same install-generation fence, joins the shared endpoint
recovery cohort while it delivers a payload, and retains the same-gateway retry
path for an unexpected outage after startup. MCP owns the normal session-long
lifecycle.
...
`Notification`, `Stop`, `PreCompact`, and `PostCompact`. Relay normalizes them
as scope, prompt, tool, mark, notification, compaction, subagent, or private LLM
correlation events according to the hook payload.
...
`UserPromptSubmit` contributes prompt and LLM correlation. `Stop` is a private
correlation and turn-boundary hint rather than a standalone user-visible mark.
The other hooks normalize to their corresponding scope, tool, mark,
notification, compaction, or subagent semantics.
