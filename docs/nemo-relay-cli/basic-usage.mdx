...
- Tool pre-use starts a NeMo Relay tool span. Tool post-use, denial, or failure
  closes it.
- An authenticated Claude Code or Codex `PermissionRequest` must match the
  recorded tool identity. Relay adds the canonical `gen_ai.tool.call.id` to
  the permission guardrail event metadata and uses the matched tool parent. If
  the permission guardrail rejects an active tool, Relay closes that tool span
  with `error.type = "guardrail_rejected"`.
