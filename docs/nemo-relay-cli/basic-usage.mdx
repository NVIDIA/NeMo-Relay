...
Use `--global` to edit `/etc/nemo-relay/config.toml`. The editor creates a missing target only
...
Use `--global` for `/etc/nemo-relay/plugins.toml`. `model-pricing add-source`
...
`PreToolUse`, `PermissionRequest`, and Hermes `pre_tool_call` hooks use
