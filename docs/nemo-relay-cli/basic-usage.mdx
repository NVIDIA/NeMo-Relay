...
Each gateway LLM start event records the exact inbound HTTP body length as
`metadata.gateway_ingress_body_bytes`. Debug logs record
`gateway_provider_body_bytes` after request interceptors and any JSON
re-encoding, immediately before the effective request is sent upstream.
