...
the same non-activating validation exposed by the Rust and Python
`plugin.validate` APIs: it reports per-plugin policy, integrity, authenticity,
and configuration-schema failures. A dynamic validation failure makes doctor
fail; live exporter and backend health checks remain separate doctor probes.
