...
that exact file. An explicit `config edit --user`, `--project`, or `--global`
flag overrides the inherited target. This selects only the file opened by the
editor; it does not change runtime discovery, layering, or merge precedence.
...
For managed LLM routes, the middleware result owns the client response. Relay
serializes a successful final JSON value with HTTP `200` and
`application/json`, or streams it with HTTP `200` and `text/event-stream`.
Successful upstream status codes, headers, and original JSON formatting are not
forwarded. This keeps the selected result coherent when an execution intercept
invokes `next` more than once, including concurrent retry or hedging attempts.
Provider and transport failures remain attached to the individual failed
attempt instead of using successful-response metadata. For ordinary managed
requests, Relay preserves the provider status, body, and response headers when
the provider returns an HTTP error. Relay also preserves an undecodable
buffered response. Retry-aware routing uses structured provider failures
internally so it can select another target. Relay uses its JSON error envelope
with HTTP `502` for transport failures.
