...
Set provider base URLs under `[upstream]` when Relay must forward compatible
requests to a proxy, enterprise endpoint, or another provider host:
...
Relay selects the configured upstream from the request path, regardless of
which compatible client sends the request.
...
Use `openai_auth_header` or `anthropic_auth_header` under `[upstream]` when an
upstream requires a complete `Authorization` value such as `Bearer ...` or
`Basic ...`. Most deployments do not need this: use `OPENAI_API_KEY` or
`ANTHROPIC_API_KEY` for standard provider API-key authentication. Use a
configured authorization value for an enterprise gateway, proxy, or compatible
endpoint that requires nonstandard authentication, including when a local
client credential must be replaced before forwarding upstream. Prefer the
corresponding environment variables to avoid storing credentials in
`config.toml`:
...
Relay preserves an inbound `Authorization`, `x-api-key`, `api-key`, or
`anthropic-api-key` value. If the request has none of those headers, Relay uses
the route-specific configured authorization value. If that value is also
unset, Relay uses `OPENAI_API_KEY` or `ANTHROPIC_API_KEY` with the provider's
standard authentication scheme.

The exception is a Codex ChatGPT OAuth-style OpenAI `Authorization` value. When
Relay has a replacement OpenAI credential—either `openai_auth_header` in the
config file or `NEMO_RELAY_OPENAI_AUTH_HEADER` in the environment—it strips the
OAuth token and forwards the configured credential to the configured OpenAI
upstream. Without a replacement, Relay retains the existing ChatGPT routing.
