...
attempt instead of using successful-response metadata. Managed provider
failures use Relay's JSON error envelope; Relay preserves an upstream error
status when one is available and uses HTTP `502` for transport failures or
