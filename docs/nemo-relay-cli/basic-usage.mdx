...
- `POST /hooks/pi` accepts the hook JSON posted by the NeMo Relay pi extension.
  It returns an empty object for an allowed call and HTTP 403 with
  `error.type = "nemo_relay_guardrail_rejected"` when a guardrail blocks one.
...
| pi | none — pi's LLM spans come from real gateway traffic, not from hooks | `session_start`, `session_shutdown`, `agent_start`, `agent_end`, `agent_settled`, `turn_start`, `turn_end`, `session_before_compact`, `session_compact`, `tool_call`, `tool_execution_end` |
