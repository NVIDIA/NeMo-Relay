...
If you change a base URL with `NEMO_RELAY_OPENAI_BASE_URL`,
`NEMO_RELAY_ANTHROPIC_BASE_URL`, `--openai-base-url`, or
`--anthropic-base-url`, set the matching auth-header environment variable when
the new endpoint needs custom authentication. Relay doesn't carry a custom
header across a URL change. Repeating the exact same URL preserves the existing
header.
