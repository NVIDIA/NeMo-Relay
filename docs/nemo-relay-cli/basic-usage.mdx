...
| pi | none yet — pi's model traffic does not reach the gateway | `session_start`, `session_shutdown`, `agent_start`, `agent_end`, `agent_settled`, `turn_start`, `turn_end`, `session_before_compact`, `session_compact`, `tool_call`, `tool_execution_end` |
