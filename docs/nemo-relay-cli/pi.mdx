...
integration is a proof of concept. Tool and turn activity reach Relay whenever
the gateway is reachable — the extension defaults to failing **open**, so an
unavailable gateway lets pi carry on and that activity is simply not recorded.
