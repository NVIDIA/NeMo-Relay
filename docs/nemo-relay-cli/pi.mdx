...
Copy the extension into `~/.pi/agent/extensions/`, or install it with
`pi install <source>` without `--local`. `nemo-relay run --agent pi` does not
need either — it passes the extension with `-e`, which loads first and is never
trust-gated.
