...
The manifest requires Relay 0.10 because this worker uses the context-aware tool execution
contract and the `LlmExecutionContext` callback shape. The protocol identifier remains `grpc-v1`. `ToolNext::call` returns
