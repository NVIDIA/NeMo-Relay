...
The worker speaks `grpc-v1`, but it also registers an LLM request intercept. That surface
requires a host range that excludes Relay 0.5, so the checked worker uses
`relay = ">=0.6,<1.0"`. Its crate dependency remains 0.8.0 because that is the SDK
version used to build this example.
