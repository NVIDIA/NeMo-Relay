...
Use typed mark options when an exported mark needs a data schema or log severity. Use
...
Use the following Python code to add schema and severity metadata to a mark, emit a
metric, and inspect the runtime diagnostics:
...
Use the following Rust code for the same operations:
