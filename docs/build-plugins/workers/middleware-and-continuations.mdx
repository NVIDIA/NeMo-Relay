...
and returns the first successful response only after both calls complete successfully.
The second call is intentional demonstration code: it can still consume provider
capacity, incur cost, and cause provider-side effects even though Relay does not expose
its result to the application. A failure from either call fails the wrapper.
