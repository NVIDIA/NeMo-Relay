...
must carry both `downstream.result` and `downstream.annotation` into its outcome. A
repeated tool continuation returns another independent structured result; it still does
not expose downstream pending marks.
