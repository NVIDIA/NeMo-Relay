...
The manifest uses `relay = ">=0.6,<1.0"` because this worker registers an LLM request
intercept. The `grpc-v1` protocol identifies the transport contract, but it does not make
an earlier Relay release compatible with a middleware surface that it did not expose. The
package still pins its own authoring dependency to the 0.8 SDK line shown above.
