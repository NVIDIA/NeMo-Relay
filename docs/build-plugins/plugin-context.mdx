...
| Runtime control | Conditional middleware guardrail | Make a matching global registration ineligible for future snapshots. Language bindings and worker SDKs use callbacks; the native SDK installs a host-resident constant reason. |
...
the target's effective global name, and the callback or native constant reason. Context methods
