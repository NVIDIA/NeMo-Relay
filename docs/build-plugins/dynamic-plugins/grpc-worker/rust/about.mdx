...
### Provide local inference

Use `register_local_model_provider` when a first-party Relay component owns the
policy and needs isolated inference:

```rust
context.register_local_model_provider("detector", |request| async move {
    Ok(serde_json::json!({
        "version": 1,
...
component owns the JSON contract, field selection, deadline, response
validation, and application. The worker should perform inference only.
