...
Event to the worker over `grpc-v1`, and the callback returns flat key/value
additions for `Event.metadata`:
...
The callback receives an immutable Event snapshot. Relay validates and merges
accepted additions before Event sanitizers run. A callback error omits that
callback's additions without dropping the Event. Stopping the worker component
