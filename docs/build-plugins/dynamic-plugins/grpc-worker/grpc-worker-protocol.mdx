...
Relay 0.8 resets the `grpc-v1` application JSON contract to use canonical
`ToolExecutionResult` values. Rebuild every worker for Relay 0.8 and declare a
...
protobuf package, services, RPCs, fields, and field numbers remain unchanged;
the JSON envelope semantics are breaking for workers built before Relay 0.8.
...
The following envelope schemas are normative for canonical tool results:

| Location | Required `JsonEnvelope.schema` |
| --- | --- |
| Successful `RelayHostRuntime.ToolNext` response `JsonResult.value` | `nemo.relay.ToolExecutionResult@1` |
| `InvokeResponse.tool_execution.outcome` | `nemo.relay.ToolExecutionInterceptOutcome@2` |

The tool intercept outcome has a flat payload containing `result`, optional
`annotation`, and Relay-owned `pending_marks`. These application JSON semantics
change without adding a protobuf package, RPC, message, or field. Workers built
against the earlier `grpc-v1` tool-result contract must be rebuilt. Future
incompatible JSON semantic changes must increment `worker_protocol`.
