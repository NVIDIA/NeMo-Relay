...
`ToolExecutionInterceptOutcome` adds Relay-owned `pending_marks`. Each opaque
JSON leaf uses `JsonValue`, whose bytes contain exactly one JSON value. This
preserves arbitrary JSON and integer precision while generated protobuf clients
enforce the surrounding result, annotation, and pending-mark structure.
...
wrapper and pending-mark structure, and `JsonValue` carries each arbitrary JSON
leaf losslessly.
