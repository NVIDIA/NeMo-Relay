...
Every LLM sanitizer invocation includes `has_active_codec` and an optional
`codec_name` alongside the request or response JSON. A two-parameter worker
handler receives that context and returns normal JSON to continue, or an empty
result to omit the LLM observability payload and annotation. The
