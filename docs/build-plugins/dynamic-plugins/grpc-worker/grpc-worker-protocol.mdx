...
description: "Understand the local `grpc-v2` contract and canonical protocol definition for NeMo Relay worker plugins."
position: 15
---
...
`grpc-v2` is the stable out-of-process plugin protocol for
...
| --- | --- | --- |
| `Handshake` | `HandshakeRequest` → `HandshakeResponse` | Confirms plugin identity, `grpc-v2`, SDK/runtime details, and supported registration surfaces. |
...
`ToolNext` keeps its existing `JsonResult` protobuf response, but `grpc-v2`
places a `nemo.relay.ToolExecutionResult@1` envelope in `value`. Tool execution
intercepts return `nemo.relay.ToolExecutionInterceptOutcome@2`, whose flat
payload contains `result`, optional `annotation`, and Relay-owned
`pending_marks`. This semantic change is why Relay rejects `grpc-v1` workers;
no protobuf package or RPC was added.
