...
For `EVENT_METADATA_INJECTOR`, Relay sets `InvokeRequest.event` to the
immutable Event snapshot. The worker returns an `InvokeResponse.json` object
whose entries are proposed additions to `Event.metadata`. The surface uses the
existing unary `Invoke` RPC; it does not add another service method. Relay
validates and merges accepted additions before Event sanitizers run. A
`WorkerError` omits that callback's additions without dropping the Event.
...
`EmitMarkRequest` keeps its original `name`, `data`, and `metadata` fields and
additively supports `data_schema` (a `nemo.relay.DataSchema@1` JSON envelope)
and canonical `severity` (`trace`, `debug`, `info`, `warn`, or `error`). An
empty severity and an absent schema preserve legacy behavior. Worker SDKs keep
the original `emit_mark(name, data, metadata)` call and expose optional fields.
The Rust and Python SDKs also provide `emit_metric` for validated
`nemo.relay.metric_measurements` marks.

`GetRuntimeDiagnosticsRequest` carries the activation ID and token. Its closed
response contains repeated `RuntimeDiagnostic { code, message, count }`
entries. Relay returns the active host-level snapshot only: it aggregates by
code, retains the latest message, saturates counts, sorts by code, and limits
the response to 32 entries. There is no per-plugin attribution. The RPC was
added without changing the `grpc-v1` handshake; an older host naturally returns
gRPC `UNIMPLEMENTED`, which SDKs translate into an unsupported-runtime-
diagnostics error.
