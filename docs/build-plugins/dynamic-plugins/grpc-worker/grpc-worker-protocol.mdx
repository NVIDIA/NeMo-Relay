...
- `Register` returns declarative subscriber, guardrail, intercept, and
  local-model-provider registrations.
...
`LOCAL_MODEL_PROVIDER` uses a JSON request and JSON result; the consuming
first-party component owns their versioned schema and publishes the provider
as `<plugin_id>/<local_name>`. `InvokeStream` emits JSON chunks or
`WorkerError` chunks.
