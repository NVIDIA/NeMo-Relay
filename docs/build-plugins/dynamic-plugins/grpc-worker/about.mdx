...
baseline. Every worker must be rebuilt and declare a `compat.relay` range that
excludes Relay versions before 0.8. Use `>=0.8.0,<1.0` unless you deliberately
need a narrower or open-ended 0.8-or-newer range. `compat.relay` is the plugin
author's compatibility assertion, not proof that an artifact was rebuilt.
