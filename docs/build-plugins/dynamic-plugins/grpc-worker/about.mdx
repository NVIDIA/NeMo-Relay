...
Worker SDKs also expose `PluginRuntime::runtime_diagnostics()` as a read-only,
host-level snapshot. Entries contain `code`, `message`, and `count`; repeated
codes are aggregated, ordered by code, and limited to 32 entries. The host does
not attribute entries to individual plugins. An older Relay host returns
`UNIMPLEMENTED`; current SDKs surface that as an unsupported-runtime-diagnostics
error without a handshake capability flag.
