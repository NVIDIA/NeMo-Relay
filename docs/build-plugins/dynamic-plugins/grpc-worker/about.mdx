...
guardrails, and intercepts through the stable `grpc-v2` protocol. The host
...
Set `compat.worker_protocol` to `grpc-v2` and include `plugin_worker` in the
...
The `grpc-v2` protocol requires `compat.relay = ">=0.8.0,<1.0"`. It carries
canonical annotated tool results and the versioned LLM request annotation
contract.
...
- Refer to [gRPC Worker Protocol Overview](/build-plugins/dynamic-plugins/grpc-worker/grpc-worker-protocol)
  when you implement `grpc-v2` without an SDK.
