...
async def decide_export(request: ExportActivationRequest) -> ExportActivationDecision:
    ...
    if request.target_kind in {
        ExportActivationTargetKind.OTLP_TRACE,
        ExportActivationTargetKind.OTLP_LOG,
        ExportActivationTargetKind.OTLP_METRIC,
    }:
        return (
            ExportActivationDecision.ALLOW
            if config.get("telemetry_allowed") is True
...
target without preventing the rest of the runtime from starting.
