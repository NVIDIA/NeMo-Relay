...
Register a global callback from `WorkerPlugin.register`. Relay sends each
Event to the Python worker over `grpc-v1`, and the callback returns flat
key/value additions for `Event.metadata`:

```python
from nemo_relay_plugin import Event, Json


def add_plugin_context(event: Event) -> dict[str, Json]:
    del event
...
The callback receives a detached Event snapshot. Changing that Python
dictionary does not modify Relay's Event; only the returned additions are
considered. Relay validates and merges accepted additions before Event
sanitizers run. An exception omits that callback's additions without dropping
the Event. Stopping the worker component removes the registration.

## Read Host Runtime Diagnostics

Use the runtime handle inside an asynchronous callback to inspect the active
host-level snapshot:

```python
diagnostics = await ctx.runtime.runtime_diagnostics()
diagnostic = diagnostics.get("otel.metric_mark_invalid")
if diagnostic is not None:
    print(f"{diagnostic.message} occurred {diagnostic.count} times")
```

`RuntimeDiagnostics.entries` is an immutable tuple ordered by code. The host
aggregates repeated codes, retains the latest message, saturates counts, and
returns at most 32 entries. Entries do not identify the emitting plugin. If a
worker connects to an older Relay host, the SDK translates gRPC `UNIMPLEMENTED`
into an unsupported-runtime-diagnostics error.

## Register an Export Activation Policy

Add `"export_activation_policy"` to the worker manifest
`capabilities.items`. Worker protocol `grpc-v1` carries the versioned request
and decision through its existing unary `Invoke` RPC:
...
async def decide_export(request: ExportActivationRequest) -> ExportActivationDecision:
    config = request.config if isinstance(request.config, dict) else {}
    country = await lookup_current_public_ip_country()
...
ctx.register_export_target(
    ExportTargetRegistration(
        ...
        activation_policy=ExportActivationPolicyConfig(
            provider="examples.python_grpc_worker",
            timeout_millis=30_000,
            config={"allowed_countries": ["US", "CA"]},
        ),
    ),
    start_custom_otlp_exporter,
)
```

Relay starts a referenced policy worker before constructing gated exporters.
It sends only the target kind and opaque policy configuration, never exporter
destinations, headers, or credentials. If the worker does not advertise the
surface, exits, errors, or exceeds the per-target timeout, Relay denies that
target without preventing the rest of the runtime from starting. The provider
above is the same worker manifest that owns `start_custom_otlp_exporter`, so the
worker starts as bootstrap infrastructure while the OTLP exporter does not
start unless its policy allows it.

## Emit Typed Marks and Metrics

Worker plugins can assign a schema and OTLP log severity to a mark without
changing the existing positional call shape. Use `emit_metric` rather than
constructing Relay's reserved metric schema directly:

```python
from nemo_relay_plugin import DataSchema, LogSeverity

await ctx.runtime.emit_mark(
    "examples.python_grpc_worker.policy_decision",
    {"allowed": True},
    data_schema=DataSchema("examples.policy.decision", "1"),
    severity=LogSeverity.INFO,
)

await ctx.runtime.emit_metric(
    "examples.python_grpc_worker.requests",
    [
        {
            ...
            "value_type": "u64",
            "value": 1,
            "attributes": {"plugin": "python-worker"},
        }
    ],
)
```

`LogSeverity` in this worker SDK comes from `nemo_relay_plugin`, whose members
are uppercase, such as `LogSeverity.INFO`. Application code uses the separate
`nemo_relay.LogSeverity` enum with title-case members such as
`LogSeverity.Warn`.

Relay validates the schema and severity at the host boundary. It validates the
metric mark after sanitization, so an invalid measurement group is rejected
atomically and is not exported as a log.
