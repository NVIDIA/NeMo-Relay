...
Set `plugin.kind` to `rust_dynamic`, `compat.native_api` to `"2"`, and
...
Native API 2 requires `compat.relay = ">=0.8.0,<1.0"`. It uses
`ToolExecutionResult` for tool continuations and preserves opaque result
annotations through synchronous and asynchronous tool intercepts. This JSON
contract is independent of the negotiated native ABI table version. The current
Rust SDK uses ABI v4, while custom ABI v2 and v3 plugins can also declare native
API 2. Relay still accepts `native_api = "1"` for existing ABI v2, v3, and v4
plugins, but that legacy raw-JSON boundary drops tool-result annotations.
