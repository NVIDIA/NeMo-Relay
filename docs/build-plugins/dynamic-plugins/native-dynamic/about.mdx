...
active. The trailing ABI-v4 diagnostics callback is negotiated through
`struct_size`; a plugin built with this SDK reports an unsupported-capability
error when an older, shorter v4 host table does not include it. Existing v4
typed-async and typed-mark capabilities remain usable with that older table.
