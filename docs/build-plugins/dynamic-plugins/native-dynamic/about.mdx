...
by `NemoRelayNativeLlmSanitizeContext`. Its `codec_kind` is `None`, `BuiltIn`,
`Runtime`, or `Opaque`. `codec_id` is a host-owned, callback-lifetime string:
it is present for `BuiltIn` (one of `openai_chat`, `openai_responses`, or
`anthropic_messages`) and `Runtime`, and null for `None` and `Opaque`. A
successful null sanitizer output omits the LLM observability payload and its
annotation. The Rust SDK converts this ABI structure into
`LlmSanitizeContext { codec: LlmCodecIdentity }` automatically.
