...
Typed native middleware requires `compat.relay = ">=0.8.0,<1.0"`. Relay rejects
its registration when the manifest admits a host that predates the SDK-owned
executor and native ABI v4. Plugins that use only synchronous registrations,
such as this example, should retain `compat.relay = ">=0.5,<1.0"`.
...
the returned futures on one SDK-owned multi-thread Tokio runtime per plugin.
The default is two workers; override `NativePlugin::executor_config` to use a
different nonzero count. This is an SDK implementation setting, not a
`relay-plugin.toml` or `plugins.toml` field:
...
Do not block executor workers. Relay scope context is installed around each
root-future poll, but child tasks created with `tokio::spawn` do not
automatically inherit it. Typed subscribers remain synchronous and continue to
run on Relay's subscriber dispatcher.
...
`Internal`, call `async_stream_is_backpressured`: retain and retry the logical
chunk or rejection only when it returns `true`. `InvalidArg` means the stream
is already closed or cancelled and must not be retried. Check
...
(one of `openai_chat`, `openai_responses`, `anthropic_messages`, or `gemini_generate_content`) and
