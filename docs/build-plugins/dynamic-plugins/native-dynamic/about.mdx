...
Register a component-owned global callback when the plugin starts. Relay calls
the callback for every Event while the component is active. The callback
receives an immutable Event snapshot and returns flat key/value additions for
`Event.metadata`:
...
context.register_event_metadata_injector(
    ...
    0,
    |_event| async move {
        let additions: BTreeMap<String, Json> = BTreeMap::from([(
            "app.source".into(),
...
The plugin can inspect the Event to decide which additions to return but cannot
modify it directly. Relay validates and merges accepted additions before Event
sanitizers run. Clearing the plugin component removes the registration.
...
Relay 0.8's current v4 host table contains the complete frozen v3 table as its
prefix. It includes typed asynchronous middleware operations such as
completion-scoped codecs and pull-based downstream LLM streams, then appends
`emit_mark_v2`, which accepts optional data-schema JSON and log severity in
addition to the legacy mark fields.
`PluginRuntime::emit_mark_with_options` uses this function, and
`PluginRuntime::emit_metric` constructs and validates Relay metric marks. The
original `emit_mark` function and the complete v1 and v3 layouts remain
unchanged.

### Runtime Diagnostics

`PluginRuntime::runtime_diagnostics()` returns a read-only host snapshot for
the active plugin configuration. Each `RuntimeDiagnostic` contains only
`code`, `message`, and `count`; `RuntimeDiagnostics::entries()` is ordered by
code and `get(code)` looks up one entry:

```rust
let diagnostics = context.runtime().runtime_diagnostics()?;
...
The snapshot is host-level, not per-plugin: Relay does not currently preserve
emitting-plugin provenance for runtime diagnostics. It aggregates repeated
codes, retains the most recently recorded message, saturates counts, and
contains at most 32 entries. It is empty when no plugin configuration is
active. ABI v4 requires the complete finalized host table. Plugins loaded
through the frozen ABI-v3 or ABI-v2 compatibility tables cannot use
`runtime_diagnostics()`.

Relay negotiates v4, then the separately frozen v3 table, then the legacy v2
table. An entry that rejects a newer table with `InvalidArg` is retried with the
next older table. Rust plugins using the typed `NativePlugin` APIs continue to
work unchanged. Raw ABI plugins can use
