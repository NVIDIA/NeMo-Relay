...
The host passes a v3 `NemoRelayNativeHostApiV3` table, whose prefix remains
compatible with `NemoRelayNativeHostApiV1`, to the entry symbol. The plugin
returns a `NemoRelayNativePluginV1` descriptor:

```rust
extern "C" fn nemo_relay_register_plugin(
    host: *const NemoRelayNativeHostApiV3,
    out: *mut NemoRelayNativePluginV1,
) -> NemoRelayStatus
```

The v3 host table retains the v2 prefix for compatibility and appends a
completion-based asynchronous middleware extension. Rust plugins using the
