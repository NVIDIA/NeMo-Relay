...
use nemo_relay_plugin::{
    ExportActivationDecision, ExportActivationTargetKind, Json, PluginContext, Result,
};

fn register_policy(context: &mut PluginContext<'_>) -> Result<()> {
    context.register_export_activation_policy(|request| async move {
        let allowed = match request.target_kind {
            ExportActivationTargetKind::OtlpTrace
            | ExportActivationTargetKind::OtlpLog
            | ExportActivationTargetKind::OtlpMetric => {
                request.config.get("telemetry_allowed") == Some(&Json::Bool(true))
            }
            _ => true,
        };
        Ok(if allowed {
