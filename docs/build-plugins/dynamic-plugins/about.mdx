...
Relay 0.8 retains the native API 1 and `grpc-v1` identifiers while resetting
their application JSON semantics to the canonical `ToolExecutionResult`
contract. Rebuild plugins with the Relay 0.8 SDKs before updating
`compat.relay`. Future incompatible JSON semantic changes must increment the
