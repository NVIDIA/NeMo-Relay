...
`compat.relay` is a normal SemVer requirement. Relay 0.8 establishes the
canonical tool-result contract for native API 1 and `grpc-v1`, so every dynamic
plugin must be rebuilt and declare a range that excludes earlier Relay
versions. Use `>=0.8.0,<1.0` unless you deliberately need a narrower or
open-ended 0.8-or-newer range. The manifest is the plugin author's
compatibility assertion; it does not attest how an artifact was built. Keep
...
the canonical `ToolExecutionResult` contract. The native boundary changes its
application JSON semantics. The worker boundary adds structured protobuf tool
result and intercept-outcome messages. Rebuild plugins with the Relay 0.8 SDKs
before updating `compat.relay`. Future incompatible contract changes must
