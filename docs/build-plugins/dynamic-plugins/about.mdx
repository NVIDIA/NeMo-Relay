...
| --- | --- | --- |
| `rust_dynamic` | Behavior must run in the Relay process. | Versioned native plugin C ABI |
...
<Warning>
Dynamic plugins that register an LLM request intercept consume the versioned
request annotation contract. For Relay 0.6 request intercepts, set
`compat.relay = ">=0.6,<1.0"`, or another range that excludes Relay 0.5. Relay
rejects the registration when the manifest still admits Relay 0.5.
</Warning>
...
| `compat` | `native_api = "1"` for the established surface. For targeted LLM continuations, use `native_api = "2"` with `relay = ">=0.8,<1.0"`. | `worker_protocol = "grpc-v1"` |
