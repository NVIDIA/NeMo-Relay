...
An export-activation provider also declares `export_activation_policy` in
`capabilities.items`. Relay recognizes that capability before activating
static observability components, starts the provider early, and evaluates all
targets that reference the provider manifest ID. A provider registers exactly
one callback; duplicate callbacks under the same manifest ID are rejected.
Other middleware and subscribers registered by that component are installed
during the same early registration and retain their configured priorities.
