...
ABI v6 adds an explicit acknowledgment that an asynchronous stream has opened.
...
| Frozen v5 extension | Context-carrying tool execution intercept registration. The callback receives one JSON object containing `tool_name`, `args`, and `tool_call_id`. |
| Current v6 extension | Explicit asynchronous stream-open acknowledgment, preserving the complete frozen v5 prefix. |
...
## Continuation Errors and Stream Opening

A native interceptor that delegates with `next.call(request).await` must return
an unhandled downstream error unchanged. The host keeps the original error for
that invocation and passes a unique reference through the SDK's string error
channel. Returning the reference restores the original error type and data.
References from another invocation, edited messages, and already-used references
are ordinary plugin errors. Do not parse or construct these references.

The host does not report a stream as established merely because its output
channel exists. ABI v6 adds `async_stream_opened`: the typed SDK calls it once
when the stream callback future returns successfully, before polling stream
items. A callback that fails while opening uses `async_stream_reject` instead.
Errors after acknowledgment remain stream-item errors.

Older SDKs need no rebuild to preserve delegated errors on a fixed host. Without
an opening acknowledgment, the host waits for the first item or clean completion
and preserves that first item exactly once. This compatibility path can delay
HTTP headers until the first event. Cancelling during opening cancels the native
output and its downstream continuations.

These corrections are targeted for Relay 0.9.1. Relay 0.8.x and 0.9.0 can convert
an upstream denial into a generic payload error or send HTTP 200 before a stream
opening failure is known. Use a build containing the correction until that
release is available. Authored `compat.native_api = "1"` remains unchanged.
