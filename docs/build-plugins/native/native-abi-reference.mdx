...
The current host requires ABI v7. It does not fall back to v6 or earlier tables because
v7 changes the LLM execution callback layouts; invoking a stale callback through the
new layout would be unsafe. Rebuild every native plugin for Relay 0.10 and raise its
