...
The callback returns `None` to leave a matching registration enabled. The callback
registration functions are an append-only native ABI v4 extension; the original
constant-reason v4 slots remain in place for existing binaries. Refer to
