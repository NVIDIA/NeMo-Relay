...
| C host-table ABI | v7 | Function table required by the current `nemo-relay-plugin` SDK. Relay 0.10 rejects older callback layouts. |

The checked example registers a tool execution intercept, so it declares
`compat.relay = ">=0.10.0,<1.0"`. Every native plugin built with this SDK must use
that lower bound because an older Relay cannot load the ABI v7 function table, even
when the component itself does not register an LLM execution intercept.

Relay 0.10 adds directional codec context to every LLM execution callback and
advances the internal table to v7. Rebuild all native plugins for this release;
the authored `native_api = "1"` label does not change.
