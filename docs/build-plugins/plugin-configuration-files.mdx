...
gateway uses internally to combine discovered layers. It is not a step operators
or language bindings invoke: configuration is written in files and applied through
plugin initialization, which performs the layering. (When the same component
`kind` appears more than once, entries pair by order of appearance, so
multi-instance plugins keep each instance rather than collapsing into the first.)
