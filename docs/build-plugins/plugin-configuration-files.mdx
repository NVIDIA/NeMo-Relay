...
| `--plugin-config-file path/to/plugins.toml` | Explicit file override for tests, demos, or wrappers that must bypass discovered plugin files. |
...
`--plugin-config` and `--plugin-config-file` are both present, if file-based
plugin config and `--plugin-config` are both present, or if `plugins.toml` and
`[plugins].config` are both present.
...
higher-precedence files define `components.config.sources`, the
...
settings in one file. Use `--plugin-config-file` for automation that should use
a specific TOML file, and `--plugin-config` for automation that should pass
inline JSON without writing files.
