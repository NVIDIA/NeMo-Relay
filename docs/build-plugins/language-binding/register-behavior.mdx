...
[priority and `break_chain`](/build-plugins/plugin-context) directly from
...
Callbacks can return additions directly or through an awaitable or Promise. Callback
failures and invalid return values omit that callback's additions without dropping the
event.
