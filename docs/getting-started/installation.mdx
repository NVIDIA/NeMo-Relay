...
Install the latest stable release from a Unix shell:

```bash
curl -fsSL https://raw.githubusercontent.com/NVIDIA/NeMo-Relay/main/install.sh | sh
```

Install the latest stable release from Windows PowerShell:
...
Install a specific version from a Unix shell:

```bash
curl -fsSL https://raw.githubusercontent.com/NVIDIA/NeMo-Relay/main/install.sh | NEMO_RELAY_VERSION=0.5.0 sh
```

In PowerShell, set the same environment variable before invoking the installer:
...
Install into another directory from a Unix shell:

```bash
curl -fsSL https://raw.githubusercontent.com/NVIDIA/NeMo-Relay/main/install.sh | sh -s -- --install-dir "$HOME/bin"
```

To choose a PowerShell installation directory, download the script and pass
`-InstallDir`:
...
| `--install-dir DIR` | Unix installer destination; defaults to `$HOME/.local/bin` or `%LOCALAPPDATA%\nemo-relay\bin` in Windows POSIX shells |
| `-h`, `--help` | Display Unix installer usage |
| `-InstallDir DIR` | PowerShell installer destination; defaults to `%LOCALAPPDATA%\nemo-relay\bin` |
| `-Help` | Display PowerShell installer usage |
...
For source installation and unsupported-platform guidance, see the
[CLI README](https://github.com/NVIDIA/NeMo-Relay/blob/main/crates/cli/README.md#installation).
