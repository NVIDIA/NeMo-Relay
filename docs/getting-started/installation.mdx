...
The installer downloads the matching GitHub Release binary to
`$HOME/.local/bin`. Verify that the command is available:
...
<Note>
The installer downloads the published SHA-256 checksum for the selected release
asset and fails without replacing an existing binary if verification does not
succeed. It does not invoke `sudo` or edit your shell configuration.
</Note>
...
matching asset exists. Native Windows and Intel macOS installations are not
supported by `install.sh`.

### Installer Reference

Set `VERSION` to a raw SemVer release tag, such as a stable or release-candidate
tag, before using the pinned examples.
...
Select the version through the environment for automation:

```bash
curl -fsSL https://raw.githubusercontent.com/NVIDIA/NeMo-Relay/main/install.sh | NEMO_RELAY_VERSION="$VERSION" sh
```
...
| Interface | Purpose |
| --- | --- |
| `VERSION` | Optional positional release tag; accepts an optional leading `v` |
| `NEMO_RELAY_VERSION` | Release tag used when no positional version is supplied |
| `--install-dir DIR` | Destination directory; defaults to `$HOME/.local/bin` |
| `-h`, `--help` | Display installer usage |

Rerun the installer to upgrade or replace the existing binary. It verifies the
new download before replacing `nemo-relay` in the selected installation directory.

### Alternative Installation Methods

Use Cargo on unsupported platforms or when you prefer to build from source:

```bash
cargo install nemo-relay-cli
```

If `cargo-binstall` is available, it can install the published release asset:

```bash
cargo binstall nemo-relay-cli
```

For PATH, permission, and unsupported-platform guidance, see the
[CLI installation notes](https://github.com/NVIDIA/NeMo-Relay/blob/main/crates/cli/README.md#installation).
