---
title: "About"
description: "Choose and implement the right NeMo Relay boundary for a framework-owned call path."
position: 1
---
...
Prefer these boundaries in order. Move down the table only when the framework
does not expose the boundary above it.

| Boundary | Use when | What Relay owns |
| --- | --- | --- |
...
A managed wrapper is the best fit when the framework exposes a stable callable
boundary. It preserves lifecycle ordering, middleware placement, timing, and
...
unless the integration invokes those standalone surfaces separately.
...
Provider-specific round-trip behavior should remain in the codec or adapter
that owns that provider shape.
