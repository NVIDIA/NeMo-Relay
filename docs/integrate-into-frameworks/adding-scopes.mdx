...
@dataclass
class RequestState:
    nemo_relay_scope: nemo_relay.ScopeHandle | None = None

def on_request_start(request_state: RequestState, request_id: str) -> None:
...
def on_request_end(request_state: RequestState, status: str) -> None:
...
- Wrap tools with [Wrap Tool Calls](/integrate-into-frameworks/wrap-tool-calls).
- Wrap model providers with [Wrap LLM Calls](/integrate-into-frameworks/wrap-llm-calls).
