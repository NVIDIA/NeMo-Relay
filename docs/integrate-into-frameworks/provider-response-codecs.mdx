...
report = nemo_relay.plugin.validate_exact(config)["config"]
if any(diagnostic["level"] == "error" for diagnostic in report["diagnostics"]):
    raise RuntimeError(report["diagnostics"])

async def main() -> None:
    async with nemo_relay.plugin.activate(config):
...
use nemo_relay::plugin::dynamic::{initialize, validate_exact};
use serde_json::json;
...
let report = validate_exact(config.clone());
...
cost-annotated. Retain the activation handle and close it during teardown if
later cases need a different resolver.
...
| Cache read / write | preserved | summed into `cached_tokens` / `total_cached_tokens` | `llm.token_count.prompt_details.cache_read` / `…cache_write` | not emitted | `gen_ai.usage.cache_read.input_tokens` / `gen_ai.usage.cache_write.input_tokens` |
...
async def invoke_provider(request: LLMRequest) -> dict[str, object]:
    return {
        "id": "chatcmpl-demo",
...
class OpenAIRoutingResponseCodec:
    ...
    def decode_response(self, response: nemo_relay.Json) -> nemo_relay.AnnotatedLLMResponse:
