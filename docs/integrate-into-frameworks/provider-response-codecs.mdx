...
Provider- or framework-reported cost remains authoritative; the resolver is
used only when `Usage.cost` is missing. Relay does not ship provider price data
...
agents, and framework integrations. The CLI commands below
