...
The `nemo-relay` gateway selects the OpenAI Chat, OpenAI Responses, and
Anthropic Messages request codecs automatically for `/v1/messages`,
`/v1/chat/completions`, and `/v1/responses`, for both buffered and streaming
calls. `GeminiGenerateContentCodec` is available for direct use with Gemini
`generateContent`-shaped payloads. Count-token, model, probe, and non-LLM
