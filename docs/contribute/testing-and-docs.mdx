...
For CI-aligned Rust test compilation and execution, select one or more owning
language scopes explicitly:
...
# Build or run one Cargo graph for several enabled languages.
...
The `rust` scope covers workspace crates other than the Python, Node.js, and FFI
binding crates. Those crates belong to the `python`, `node`, and `go` scopes.
Supplying several graph scopes deduplicates them into one Cargo selection. The
first argument to `run-rust-tests` is the owner to execute; its remaining
arguments must match the graph passed to `build-rust-tests`. Use the `all` scope
as shorthand for the complete workspace graph.
