...
| `governor.epsilon` | `1.0` | Positive finite initial sensitivity threshold. |
...
When the governor is enabled, low-sensitivity learned hints may be skipped
under load. Manual latency-sensitivity overrides still force hint injection for
that request.
...
- `governor.epsilon` is not positive and finite.
