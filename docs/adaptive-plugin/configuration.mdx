...
| `agent_context` | Scope-local agent context propagation behavior. |
...
The requested area pages cover [Adaptive Cache Governor (ACG)](/adaptive-plugin/acg),
[Adaptive Hints](/adaptive-plugin/adaptive-hints), and
[Agent Context](/adaptive-plugin/agent-context). State, telemetry, tool
parallelism, and policy remain whole-plugin settings:
...
adaptive_config = nemo_relay.adaptive.AdaptiveConfig(
    ...
    agent_context=nemo_relay.adaptive.AgentContextConfig(
        inject_body_path="nvext.agent_context",
    ),
...
adaptiveConfig.agent_context = adaptive.agentContextConfig({
  inject_body_path: "nvext.agent_context",
});
...
adaptive.agent_context = Some(AgentContextComponentConfig {
    inject_body_path: "nvext.agent_context".into(),
...
use nemo_relay_adaptive::{
    ...
    AgentContextComponentConfig, BackendSpec, StateConfig, TelemetryComponentConfig,
    ToolParallelismComponentConfig,
};
