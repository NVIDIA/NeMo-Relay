...
The v1 contract accepts flat dotted keys under the `nv.*` namespace. Values may
be strings, numbers, booleans, or homogeneous lists of those primitive types.
Nested objects, `null`, mixed-type lists, keys outside `nv.*`, and keys with
empty dotted segments are rejected as one atomic callback result.
