...
Use these comparisons to pick the middleware surface that matches the behavior
you need:
...
This ordering preserves the semantic split between the families:
...
Relay does not run request intercepts, execution intercepts, or the real
callback. It sanitizes and enqueues the rejection mark, then returns the
rejection to the managed caller.
...
flowchart LR
    ...
    Request --> Conditional -->|rejected| Mark --> Rejected
```
