...
`context` has this shape in every language binding:
...
`has_active_codec` distinguishes a legacy/manual call with no codec from a
managed call using an unrecognized custom codec. `codec_name` is present only
for Relay's built-in `openai_chat`, `openai_responses`, and
`anthropic_messages` codecs. Do not infer a provider from a custom request
shape.
...
register_llm_sanitize_request_guardrail(
    ...
    Arc::new(|mut request, context| {
        if context.codec_name.as_deref() == Some("openai_chat") {
...
def redact_request(request: LLMRequest, context: dict) -> LLMRequest | None:
...
relay.registerLlmSanitizeRequestGuardrail(
  ...
  10,
  (request, context) =>
    context.codec_name === "openai_chat"
