...
Event metadata injectors fail open. If an injector fails, panics, or returns
invalid attributes, Relay logs the failure, ignores only that injector's output
for the current Event, and continues with later injectors using the Event as it
stood before the failed injector.
...
The v1 contract accepts keys made of non-empty segments containing only ASCII
letters, digits, underscores, or hyphens. Segments may be separated by single
dots, so keys such as `region.zone.name` are valid. Leading dots, trailing
dots, consecutive dots, whitespace, and unsupported punctuation are invalid.
Relay stores accepted keys literally; dots do not create nested objects. Values
may be strings, numbers, booleans, or homogeneous lists of those primitive
types. Nested objects, `null`, and mixed-type lists are rejected as one atomic
callback result.
    ...
    Arc::new(|event| {
        Box::pin(async move {
            let attributes = if event.name().starts_with("tool") {
                BTreeMap::from([("example.tool.category".into(), json!("catalog"))])
            } else {
                BTreeMap::new()
            };
            Ok(attributes)
