...
## Contextual LLM Sanitizers

Every LLM sanitize guardrail receives the payload first and a required
per-call context second:

```text
request:  (LlmRequest, context) -> Option<LlmRequest>
response: (Json, context) -> Option<Json>
```

`context.codec` is a binding-native codec identity object. Its `kind` and,
when applicable, `id` fields are available as ordinary Rust, Python, Node, Go,
or native-plugin properties rather than as a JSON payload.

`codec.kind` is `none` for a legacy/manual call with no codec, `builtin` for
...
Rust, Go, raw C FFI, native plugins, and Rust worker plugins must implement
this two-parameter contract. Python accepts an inspectable one-parameter
callback as a compatibility form and supplies no context to it. Node.js always
calls the callback with two arguments; JavaScript one-parameter callbacks
naturally ignore the extra argument. In both languages, new callbacks should
...
register_llm_sanitize_request_guardrail(
    ...
    Arc::new(|mut request, context| {
        if context.codec == LlmCodecIdentity::BuiltIn(BuiltinLlmCodec::OpenAiChat) {
            request.content["messages"] = serde_json::json!([]);
        }
        Some(request)
    }),
)?;
...
def redact_request(request: LLMRequest, context: nemo_relay.LlmSanitizeContext) -> LLMRequest | None:
    if context.codec.kind == "builtin" and context.codec.id == "openai_chat":
        return LLMRequest(request.headers, {"messages": []})
    return request
...
relay.registerLlmSanitizeRequestGuardrail(
  ...
  10,
  (request, context) =>
    context.codec.kind === "builtin" && context.codec.id === "openai_chat"
      ? { ...request, content: { ...request.content, messages: [] } }
      : request,
);
```
