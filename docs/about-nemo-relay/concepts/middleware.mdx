...
or sanitize emitted `Event` objects. NeMo Relay applies each middleware type at
a specific lifecycle point.
...
for the current `Event`, and continues with later injectors using the `Event` as
it stood before the failed injector.
...
stores accepted keys literally; dots do not create nested objects. Values may
be strings, numbers, booleans, or homogeneous lists of those primitive types.
Nested objects, `null`, and mixed-type lists are rejected as one atomic callback
result.
