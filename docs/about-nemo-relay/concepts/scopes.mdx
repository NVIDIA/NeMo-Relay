...
subscribers. The fork also preserves the captured OpenTelemetry parent, so its
work continues the parent's exported trace.
...
context contains an immediate `parent_uuid` and an optional `root_uuid`.
Rooted contexts carry a root UUID; explicit rootless contexts carry `None`.
By default, Relay uses the propagated root or the first local Agent scope as
that root.
...
The default `capture_propagation_context()` API preserves Relay event parentage
and includes the exact active OpenTelemetry context when one is available. When
no valid W3C parent is available, Relay can reconstruct a Relay-derived trace
from the root UUID. An explicit rootless context stays rootless until a local
Agent scope establishes a new root. Use
`capture_rootless_propagation_context()` only when the receiver must start a new
Relay-derived trace and no W3C parent is available.
`capture_propagation_context_with_root()` lets an application choose a stable
run UUID explicitly.

Relay's JSON context can also carry optional W3C `traceparent` and
`tracestate` fields. When a receiver imports a valid pair, its first local
OpenTelemetry span is a child of the upstream span and preserves upstream
sampling and vendor state; Relay UUID lineage remains unchanged. Invalid W3C
fields are ignored with a runtime warning, without rejecting the Relay context.

For outbound-only W3C propagation, use `capture_traceparent()` to obtain the
current Relay traceparent directly, or convert a `PropagationContext`
with `PropagationContext::to_traceparent` (and the equivalent binding methods).
When valid W3C context is present, Relay emits its W3C headers even for a
rootless Relay context; rootless only starts a new Relay-derived OpenTelemetry
trace when no W3C parent is available. Managed LLM execution automatically replaces both
runtime-owned `traceparent` and `tracestate` headers on the provider request;
a standalone request-intercept call adds the same pair only when a real Relay
