...
1. Relay pushes an `Agent` scope beneath the root and emits its start event.
...
3. Relay pushes a `Tool` scope, runs the tool, emits the tool events, and pops
   the scope. The agent scope becomes active again.
4. Relay pushes an `Llm` scope, runs the model call, emits the LLM events, and
   pops the scope.
5. Relay emits the agent end event and pops the agent scope. Its local
...
flowchart LR
  subgraph Stack["Active scope stack over time"]
    direction LR
    s1["root → agent"] --> s2["root → agent → tool"]
    s2 --> s3["root → agent"]
    s3 --> s4["root → agent → llm"]
    s4 --> s5["root"]
  end
...
| Work crossing a process boundary | Carry Relay propagation context |
