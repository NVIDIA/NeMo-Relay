...
subscribers. The fork also preserves the Relay observability root, so its work
continues the parent's OpenTelemetry trace.
...
and, when a root is available, OpenTelemetry trace continuity. Forking an
explicit rootless context keeps it rootless until a local Agent scope establishes a new root. Use
`capture_rootless_propagation_context()` only when the receiver must start a
new trace. `capture_propagation_context_with_root()` still lets an application
choose a stable run UUID explicitly.
