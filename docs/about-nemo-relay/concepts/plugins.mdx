...
Plugin initialization owns the registrations it installs. Clearing active
...
that should remain active across requests in one process. It is not permanent:
...
| Host-registered | An embedding application links a component crate and registers its kind before validation. | A direct host controls its linked feature set. |
...
1. `plugins add` records the manifest and prepares managed worker resources when
   required.
2. `plugins validate` checks compatibility, schema, policy, and trust evidence.
3. `plugins enable` changes desired state; the plugin loads during the next host
   startup.
4. `plugins disable` keeps the registration but prevents it from loading on the
   next startup.
5. `plugins remove` deletes the registration and any Relay-managed worker
   environment after the running host has stopped.
