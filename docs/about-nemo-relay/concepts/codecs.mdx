...
payloads remain observable as raw lifecycle data, but no provider annotation is
invented.
...
and request-affinity hints through host-specific payload shapes. These facts
help NeMo Relay attach lifecycle events to the right scope, but they do not
decode provider schemas.
