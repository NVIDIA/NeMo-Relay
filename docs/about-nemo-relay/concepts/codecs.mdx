...
- `api_specific` is a tagged, mutable surface for modeled Anthropic Messages,
  OpenAI Chat Completions, or OpenAI Responses request controls. Response
  annotations use the same pattern for provider-only metadata, including Gemini
  thinking tokens.
...
generateContent, each own the logic that recognizes their request and response
shapes and maps them into `AnnotatedLlmRequest` or `AnnotatedLlmResponse`.
