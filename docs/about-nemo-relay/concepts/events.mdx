...
- **Stale history:** Later LLM starts retain all system instructions, the latest
  user message, and every following assistant or tool message. If the history
  has no user message, the annotation retains system instructions and the final
  non-system message.
...
- **Codec projection:** When a request codec is active, Relay applies the same
  history projection to the event's provider-shaped input. If that event-only
  encode fails, Relay retains the complete event input and annotation. The
  request used for provider execution remains unchanged.
