...
tool loads a complete `SKILL.md` file. The following payload excerpt shows the
stable skill-load fields. Standard fields from the [shared envelope](#shared-envelope),
such as `atof_version`, `uuid`, `parent_uuid`, and `timestamp`, are omitted.
...
| Source | Detected Load |
| --- | --- |
| `skill_tool` | A first-class skill tool, such as `Skill` or `skill_view` |
| `structured_read` | A whole-file structured read |
| `shell_read` | A standalone, complete `cat`, `bat`, `batcat`, or PowerShell `Get-Content` read |

The path must end in `SKILL.md`; the skill name is its immediate parent
directory. Structured offsets or limits and shell pipelines, redirections,
compound commands, and range-limited readers do not count. Relay omits the full
path and shell command from the mark.
...
- It records an eager load attempt and remains present if the read later fails.
- A blocked call that never starts does not emit the mark.
- Relay emits each skill once per tool call. A later call that loads the same
  skill emits another mark.
