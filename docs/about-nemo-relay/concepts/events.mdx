...
Detection affects observability only. An unsupported or unrecognized read still
...
The three related facts are intentionally distinct: `skill.load` records an
attempt detected before execution, the tool end event records whether execution
succeeded, and `skill.load.inferred` records a prompt expansion that Relay could
not confirm as a file read. Consumers should not treat any one of them as proof
of the others.
...
ATOF export writes this raw canonical event stream. ATIF and the typed
