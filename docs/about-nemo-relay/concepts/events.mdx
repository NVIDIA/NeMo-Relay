...
nonzero `offset`, a `limit`, or a range control. It also doesn't emit a mark for
shell commands that use pipelines, redirections, compound commands, or
range-limited readers. An explicit structured `offset` of `0` still represents
a complete read. Relay omits the full path and shell command from the mark.
