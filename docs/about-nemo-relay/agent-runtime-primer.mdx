...
separately distributed plugin package should install team-owned or third-party
...
Events form a different shape. Relay emits parent-linked start and end events
for the Agent, Tool, and LLM scopes. Those records remain in an event tree after
the active scope has closed. A mark records a point-in-time fact under the
active scope; it does not add another entry to the scope stack.
...
Codecs are optional boundary translators. They normalize typed application
values or provider-native payloads when middleware, events, or exporters need a
stable runtime shape; they are not a required step in the execution model.
