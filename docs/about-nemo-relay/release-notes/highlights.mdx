...
NVIDIA NeMo Relay 0.4 adds host plugin installation, first-party PII redaction,
local Guardrails execution, observability with model pricing, streaming
raw-event export, and stronger coding-agent trace fidelity.
...
- Model pricing estimates require an inline, file-backed, or embedding-provided
  model pricing source. NeMo Relay does not ship provider price data by default.
...
- Added model pricing source management commands for validating catalogs, adding
