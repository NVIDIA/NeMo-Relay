...
- Codex can perform provider discovery and request `/models` before launching
  ...
  readiness.
- On Windows, a restrictive host Job Object can prevent the Codex plugin's
  Relay sidecar from breaking away. When nested job assignment is supported,
  the sidecar remains scoped to the host job and the shared gateway might not
  remain available for the full 300-second idle reuse window. If Relay cannot
  create or configure its cleanup job, bootstrap fails instead of starting
  without descendant-cleanup guarantees.
