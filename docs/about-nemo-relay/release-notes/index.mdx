...
- Relay 0.8 resets native API 1 and `grpc-v1` application JSON semantics to the
  canonical `ToolExecutionResult` contract. Rebuild every native and worker
  plugin and declare a `compat.relay` range that excludes Relay versions before
  0.8. This tool-result cutover changes neither the native ABI v4 table nor the
  `nemo.relay.worker.v1` protobuf transport layout.
