...
requires a type and endpoint for every OpenTelemetry exporter, and removes
configurable mark and attribute-alias controls. Refer to
...
environment variables through `header_env`. NeMo Relay validates the variable
and snapshots its value when the plugin activates.
