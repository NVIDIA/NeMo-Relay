...
- The PII redaction plugin supports deterministic built-in policies and
  worker-backed local-model providers. Local-model mode requires a separately
  installed compatible `grpc-v1` provider; the optional Rampart provider
  supports Latin-script text and does not provide complete PII coverage.
