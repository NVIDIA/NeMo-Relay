...
The `nemo-relay-node` package still requires Node.js 24 or later, but the
platform-specific native packages no longer declare an `engines` range. npm
omits an optional dependency whose `engines` do not match the running Node.js,
which previously left `require("nemo-relay-node")` unable to find its native
binary on older hosts. With npm's default `engine-strict=false` setting,
installs on Node.js 22 and 23 now resolve and load the platform package, and the
only signal that the runtime is below the supported floor is the `EBADENGINE`
warning npm prints for the metapackage. With `engine-strict=true`, or a package
manager that enforces the parent package's engine range, installation fails.
Node.js 22 and 23 remain unsupported.
...
- The Node.js binding and package workflows require Node.js 24 or later. Under
  npm's default `engine-strict=false` setting, the native packages install and
  load on Node.js 22 and 23, and an `EBADENGINE` warning on the metapackage is
  the only signal a user gets. With `engine-strict=true`, or a package manager
  that enforces the parent package's engine range, installation fails. Node.js
  22 and 23 remain unsupported.
