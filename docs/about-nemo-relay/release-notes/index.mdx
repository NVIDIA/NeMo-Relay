...
binary on older hosts. Installs on Node.js 22 and 23 now resolve and load the
native package, and the only signal that the runtime is below the supported
floor is the `EBADENGINE` warning npm prints for the metapackage.

Each platform-specific native package now declares a JavaScript entry point that
loads its `.node` binary, instead of declaring the binary itself as the package
entry point. Runtimes that do not resolve a `.node` file as a package entry
point, such as the Bun runtime, can now load `nemo-relay-node` by name.
...
- The Node.js binding and package workflows require Node.js 24 or later. The
  native packages install and load on Node.js 22 and 23, but those versions are
  unsupported, and an `EBADENGINE` warning on the metapackage is the only signal
  a user gets.
