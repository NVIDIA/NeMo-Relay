...
Common install options become hook-forwarding command arguments and gateway
headers:
...
`NEMO_FLOW_GATEWAY_URL` and run:

```bash
nemo-flow hook-forward <agent>
```

Persistent installer output embeds `--gateway-url` and any selected export or
...
discovers the gateway through `NEMO_FLOW_GATEWAY_URL`; in persistent installs
you can still pass `--gateway-url`. It fails open by default so observability
