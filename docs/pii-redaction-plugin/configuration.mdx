...
| Built-in actions | `remove`, `regex_replace`, `hash`, `mask` | N/A |
...
- `builtin.action` must be `remove`, `regex_replace`, `hash`, or `mask`.
...
| Field | Purpose |
|---|---|
| `action` | Sanitization action. Current values are `remove`, `regex_replace`, `hash`, and `mask`. |
...
| `detector` | Optional built-in matcher preset. Current values are `email`, `phone`, `api_key`, `ip_address`, and `url`. |
...
| `unmasked_prefix` | Leading character count to keep when `action = "mask"`. Defaults to `0`. |
| `unmasked_suffix` | Trailing character count to keep when `action = "mask"`. Defaults to `0`. |
...
The built-in detector presets are intended for common structured values:
