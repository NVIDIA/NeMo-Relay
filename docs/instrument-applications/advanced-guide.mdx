...
Node.js guardrail and request-intercept callbacks are synchronous. A thrown
conditional-execution guardrail or request intercept rejects the managed call
before protected execution or later middleware runs. A thrown sanitize guardrail
or event sanitizer fails open, preserving the current emitted payload and
recording the error for `getLastCallbackError()`.
