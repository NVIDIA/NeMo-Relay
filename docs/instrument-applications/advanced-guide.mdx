...
Complete [Instrument a Tool Call](/instrument-applications/instrument-tool-call) or [Instrument an LLM Call](/instrument-applications/instrument-llm-call). Middleware only runs when the call goes through a NeMo Flow managed lifecycle API.
...
- Use [Middleware](/about-nemo-flow/concepts/middleware) to review execution order.
