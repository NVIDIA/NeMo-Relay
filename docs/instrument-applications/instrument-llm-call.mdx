...
recent explicit `Agent` scopes in the active scope stack; the implicit root
scope is ignored. Turn scopes are always `Custom` scopes and do not change
agent lineage. Harness session metadata is used when present; application
scope names are used otherwise. No plugin or configuration is required.
...
Native subscriber delivery is asynchronous. Flush subscribers before validating
