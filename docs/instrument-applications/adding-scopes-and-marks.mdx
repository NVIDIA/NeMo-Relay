...
in Rust, `scope.metric()` in Python, `metric()` in Node.js, or `EmitMetric()`
in Go. See
[Emit Log and Metric Marks](/configure-plugins/observability/opentelemetry#emit-log-and-metric-marks)
...
- Export severity-tagged marks and metric measurements with
  [OpenTelemetry](/configure-plugins/observability/opentelemetry#emit-log-and-metric-marks).
