...
- **Metric routing semantics.** Whether the Relay-owned `nemo.relay.metric_measurements` schema should become an ATOF-wide convention, including schema ownership, versioning, timestamps, aggregation, and exemplars. In ATOF `0.1`, `data_schema` remains opaque and ATOF does not normalize metrics.
