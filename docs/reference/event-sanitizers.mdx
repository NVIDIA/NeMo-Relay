...
input/output lifecycle meaning, and schemas cannot be changed through this
API. Set a field to `null` or `None` to remove it. Preserve fields that your
policy does not change.

Registries are priority ordered. Lower priorities run first, and each callback
receives the fields returned by the preceding callback. Invalid callback
results fail open and preserve the current fields. Binding-specific callback
error inspection remains available where the binding exposes it.
