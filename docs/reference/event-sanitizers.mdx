...
there is no implicit timeout. A callback that returns `Pending` must settle the
handle exactly once, or serial event publication remains blocked. Relay cancels
the handle when the invocation is abandoned; late or duplicate settlement after
