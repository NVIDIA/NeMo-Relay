...
| pi | 0.84.0 | Transparent run through a Relay-authored pi extension, 11 lifecycle hooks, and tool-call security | Proof of concept. No persistent install: pi has no plugin marketplace. Model traffic does not traverse the gateway, so there are no LLM spans and no model-call enforcement. Subagents and nested pi processes appear as unrelated sessions. pi ships breaking changes through minor releases, so hook signatures need re-verification after an upgrade. |
...
Code](/nemo-relay-cli/claude-code), [Codex](/nemo-relay-cli/codex), and
