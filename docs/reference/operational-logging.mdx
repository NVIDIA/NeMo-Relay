...
When none of these variables are set, Python, Node.js, and Go defer to an
existing application logger if it already owns Rust's process-global `log`
facade. Set one of these variables when Relay must configure its own logging.
