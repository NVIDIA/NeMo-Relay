...
When Relay layers `config.toml` files, the exact configured `path` is the file
sink identity. Distinct paths aggregate in system, project, then
explicit-or-user order. For matching paths, higher-layer fields recursively
overlay lower-layer fields, producing one effective sink. Path aliases such as
`relay.log` and `./relay.log` are distinct identities.
