...
canonical ATOF event stream. Subscribers consume that stream in process, and
exporter-oriented subscribers write raw ATOF JSONL or translate events into
ATIF, OpenTelemetry, or OpenInference.

The first-party plugin component has kind `observability`. It can install:

- ATOF JSONL export for raw lifecycle events.
- ATIF trajectory export for each top-level agent scope.
...
| Need | Use |
|---|---|
| Raw canonical event stream | [ATOF](atof.md) |
| Offline analysis, replay, or evaluation trajectories | [ATIF](atif.md) |
...
- [ATOF](atof.md) covers raw JSONL event stream export.
- [ATIF](atif.md) covers per-agent trajectory export.
