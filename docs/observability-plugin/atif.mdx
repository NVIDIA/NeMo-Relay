...
Point-in-time mark events become `system` steps: the step `message` is the
mark's hook event name when present, otherwise the mark name, and the mark
payload, metadata, semantic category, and subtype are preserved under
`extra.event_payload`, `extra.event_metadata`, `extra.event_category`, and
`extra.event_subtype`. A mark is recorded whenever it carries a payload,
metadata, or a category, so a mark stays discoverable even without a payload.
