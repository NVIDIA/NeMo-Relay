...
| `output_directory` | Current working directory | Directory containing trajectory files. Ignored when `storage` is set. |
| `filename_template` | `nemo-relay-atif-{session_id}.json` | Must contain `{session_id}`. |
| `storage` | Omitted | Optional remote storage destination. When set, trajectories are uploaded to the configured backend instead of being written locally. See [Remote storage](#remote-storage). |

## Remote storage

For sandboxed runtimes such as NemoClaw / Omnistation / OpenShell, where local
trace files are not durable across sessions, configure the `storage` block to
ship completed trajectories directly to object storage. When `storage` is set
the local file write is replaced by an upload; `output_directory` is ignored.

The block is tagged with a `type` discriminator so additional backends can be
added without breaking existing configs. Today, S3-compatible object storage is
supported.
...
environment variables and are deliberately not surfaced in the configuration so
secrets cannot end up in checked-in config files:

| Environment Variable | Purpose |
|---|---|
| `AWS_ACCESS_KEY_ID` | Static access key (or use `AWS_SESSION_TOKEN`-based credentials). |
| `AWS_SECRET_ACCESS_KEY` | Static secret key. |
| `AWS_SESSION_TOKEN` | Optional session token for temporary credentials. |
| `AWS_REGION` | Bucket region. |
| `AWS_ENDPOINT_URL` | Optional endpoint override for S3-compatible storage. |
| `AWS_ALLOW_HTTP` | Set to `true` when targeting an HTTP endpoint. |
...
If an upload fails, the failure surfaces through the dispatcher's last-error
result on plugin teardown; subsequent trajectory writes for the same dispatcher
session are short-circuited.
...
- The application never opens a top-level `agent` scope, so no trajectory file
  is created.
- `storage.type` is unknown or `storage.bucket` is empty.
- `storage` is set in a build that was compiled without the `atif-storage`
  feature, or on a target (such as WebAssembly) where remote storage is not
  supported.
