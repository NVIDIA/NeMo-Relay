...
| Failure | Behavior |
|---|---|
| Invalid `plugins.toml`, duplicate component kinds, malformed component shapes, unsupported values, or unavailable exporter features | Validation or initialization fails. If a previous plugin configuration was active, NeMo Relay attempts to restore it after a failed replacement. |
| ATOF file open, serialization, or flush failure | Application work continues. The exporter stores the failure, stops accepting later events for that file, and returns the stored error from `force_flush()` or `shutdown()`. |
| ATOF streaming endpoint startup or send failure | File output and other endpoints continue. Endpoint failures are logged with the endpoint index; endpoint flush and close timeouts are logged instead of blocking shutdown indefinitely. |
...
Use `nemo-relay doctor` and exporter-specific logs when validating remote
collector paths such as OTLP, Intake-connected HTTP endpoints, or object
storage. For local artifact paths, verify that the running process can create
the output directory and that teardown calls `plugin.clear()` or
