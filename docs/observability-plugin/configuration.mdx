...
| `atof_stream` | Registers a global TCP-backed ATOF JSONL stream exporter to a separate receiver process. |
...
- Streaming Agent Trajectory Observability Format (ATOF): `__nemo_relay_plugin__observability__atof_stream`
...
await plugin.initialize({
  ...
  components: [
    observability.ComponentSpec({
      ...
      atof_stream: observability.atofStreamConfig({
        enabled: true,
        address: "127.0.0.1:43199",
      }),
...
use nemo_relay::observability::plugin_component::{
    AtifSectionConfig, AtofSectionConfig, AtofStreamSectionConfig,
...
let component = ComponentSpec::new(ObservabilityConfig {
    ...
    atof_stream: Some(AtofStreamSectionConfig {
        enabled: true,
        address: Some("127.0.0.1:43199".into()),
    }),
