...
description: "Configure the built-in Adaptive component, including state, telemetry, hints, ACG, and response caching."
position: 2
---
...
| `response_cache` | Exact-match LLM response caching. Requires a non-empty trust-domain `namespace`. |
...
Treat `response_cache.namespace` as a security boundary, not only a key prefix.
One response-cache configuration must not span mutually untrusted tenants or
upstreams. Use separate Relay gateway/cache deployments for those trust domains,
or leave response caching disabled. Validation rejects an empty or whitespace-only
namespace for both in-memory and Redis backends.

When Switchyard and response caching are enabled together, keep
`switchyard.priority` lower than `response_cache.priority` (the defaults are
`0` and `50`). Relay rejects equal or inverted priorities because the selected
backend must be part of the cache key.

<Note>
Rust 0.7 adds `response_cache` to the public `AdaptiveConfig` struct. Existing
exhaustive struct literals must add `response_cache: None`. Prefer setting the
fields you need and ending the literal with `..AdaptiveConfig::default()` so
future optional fields keep their defaults.
</Note>

Streaming cache publication is write-behind. Relay reports stream completion
without waiting for the cache backend to confirm the write, so an identical
request issued immediately after completion can still run live. A later request
can reuse the entry after publication finishes. This keeps Redis latency or an
unavailable cache backend from delaying the streamed response.
