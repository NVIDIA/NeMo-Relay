...
activation = asyncio.run(nemo_relay.plugin.initialize(plugin_config))
# Retain the activation while adaptive behavior is needed, then close it during teardown.
asyncio.run(activation.close())
```
</Tab>
...
let activation = initialize(plugin_config, None).await?;
...
Validate plugin configuration before initialization. Disabled components are
excluded from the effective host; validate their component-local configuration
before enabling them when preparing a rollout.
