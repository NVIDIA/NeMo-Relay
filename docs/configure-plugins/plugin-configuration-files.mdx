...
dynamic_plugins = plugin.load_dynamic_plugin_activation_specs("path/to/plugins.toml")
...
enablement. Every `[[plugins.dynamic]]` declaration in the selected file is
returned, so passing the result to `initialize_with_dynamic_plugins()` is
explicit consent to load those trusted native libraries or worker processes.
