...
### Embedded Python Host Initialization

Python hosts pass an optional explicit `plugins.toml` directly to the unified
host initializer:
...
Core resolves manifest references relative to the declaring file, applies
lifecycle state and trust policy, validates all selected plugins before loading
code, and owns every resulting runtime through the returned activation handle.
Use `plugin.activate(...)` when the host lifetime fits one `async with` block.
Use `await plugin.initialize(...)` when the caller needs to retain and close the
activation manually.
...
`atof.sinks`, `opentelemetry.endpoints`, `opentelemetry.logs.endpoints`,
`opentelemetry.metrics.endpoints`, and `atif.storage`. For example,
...
those file entries. The same higher-precedence-first order applies to
`config.opentelemetry.logs.endpoints` and
`config.opentelemetry.metrics.endpoints`.

For a log or metric signal, an explicitly empty higher-precedence `endpoints`
list clears inherited explicit signal endpoints. An enabled signal then fails
validation because its explicit list is empty. Omit the `endpoints` field to
derive destinations from trace endpoints instead.
