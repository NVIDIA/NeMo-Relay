...
the plugin stays disabled. Python workers are an exception: their package setup
can run code, so Relay rejects the install before setup if the host does not
trust the bundle. Set the trust rule first, then retry the install.
