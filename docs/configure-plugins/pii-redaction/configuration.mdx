...
export. It replaces present opaque JSON payloads with `{}`, including raw LLM
and tool input/output, headers, generic event data and metadata,
provider-native values, category-profile extras, and unknown custom-mark
payloads. A valid `nemo_relay.log.severity` value is restored for marks.
...
unapproved numbers become `0`, and unapproved booleans become `false`. Routing
metadata, pricing provenance, optimization contribution evidence, and unknown
fields are discarded. Relay lifecycle identifiers remain unchanged.
