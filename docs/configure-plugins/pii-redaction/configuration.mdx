...
| `target_path_globs` | Restricted JSON-pointer glob paths. A whole `*` segment matches one object key or array index; a whole `**` segment matches zero or more. |
...
object key or array index, while a whole `**` segment matches zero or more
segments. Exact paths and glob paths are combined.
Within `target_path_globs`, `*` and `**` are wildcards only when each is a
complete pointer segment. Segments such as `content*` and `foo*bar` match
those literal object keys.
...
`/output/*` selects only a direct child of `output`. It does not select
`/output/0/content/0/text`; use `/output/*/content/*/text` for that shape.
Use `/messages/**` to select `messages` and every descendant, including nested
provider-native values. Relay does not support predicates or arbitrary JSONPath
expressions. `target_paths = ["/*"]`
continues to select a literal `*` object key; use `target_path_globs = ["/*"]`
when `*` should be a wildcard.

Use `**` with `action = "redact"` and a whole-string pattern when the policy
must redact every string value below a stable root while preserving objects,
arrays, and field names. This also redacts structural string values such as
roles, types, and identifiers.
