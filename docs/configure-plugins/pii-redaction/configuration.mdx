...
`local_model` is reserved for a future host-provided local-model backend.
...
The optional
[Rampart worker](/configure-plugins/pii-redaction/rampart-worker) does not
register through this provider seam. It runs as a manifest-backed `grpc-v1`
dynamic plugin so its ONNX runtime, model cache, and process lifecycle remain
outside the Relay host.
