...
For an out-of-process contextual detection option, use the optional
[Rampart worker](/configure-plugins/pii-redaction/rampart-worker). It follows
the manifest-backed `grpc-v1` worker lifecycle and does not install ONNX
dependencies into the Relay host.
...
- [PII Redaction Configuration](/configure-plugins/pii-redaction/configuration)
  ...
  and example configs.
- [Rampart Worker](/configure-plugins/pii-redaction/rampart-worker) documents
  optional local contextual detection and how it composes with deterministic
  profiles.
