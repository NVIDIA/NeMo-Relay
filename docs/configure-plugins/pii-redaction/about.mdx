...
replaces the entire emitted LLM body rather than applying normalized selectors
to an incompatible raw provider shape.

## Current Boundaries

This plugin is intentionally scoped to a deterministic built-in backend plus a
local worker-provider extension point.

In particular:

- Local-model providers are optional dynamic plugins. They register a
...
- `allow_network = false` prohibits network inference by contract, but the
  worker process is not a network sandbox. Install only trusted providers.
