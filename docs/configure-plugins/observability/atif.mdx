...
By default, marks use exporter-native handling. In ATIF, marks with payloads
remain point-in-time `system` steps with their payload, category, category
profile, timestamp, and ancestry preserved in `extra`. Set
...
not remove mark payload or metadata. ATIF suppresses the built-in high-volume
`llm.chunk` stream receipts in `inherit` mode and when they are excluded from
`tool` projection. `event` explicitly includes payload-bearing `llm.chunk`
marks as system steps.
