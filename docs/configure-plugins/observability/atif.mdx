...
| `header_file` | `{}` | Maps header names to regular files read before each completed-trajectory upload. Files must exist at activation. Relay removes trailing whitespace. Remote destinations must use HTTPS. HTTP is allowed only for localhost and loopback IPs. Relay does not follow redirects, so a redirect fails that upload and does not send the file-backed value to the new destination. |
