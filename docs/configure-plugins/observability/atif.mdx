...
| `local_activation_policy` | Omitted | Optional policy for the implicit local file and local recovery fallback. A denial suppresses both uses. |
...
Each HTTP or S3 entry can include `activation_policy`. Relay evaluates entries
independently and preserves their original configuration indices in
diagnostics. If every configured remote entry is denied, Relay disables the
dispatcher instead of promoting the implicit local destination to primary
storage.
