...
value is unsafe, Relay skips that trajectory and logs
`atif_destination_render_failed`. The rendered filename applies to local, S3,
and HTTP storage in the same way as a static filename.
