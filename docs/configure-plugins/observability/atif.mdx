...
The diagnostic remains visible in the active plugin report and causes plugin
teardown to report the degraded delivery, even when the local recovery write
succeeds. Fatal dispatcher failures, such as trajectory serialization failures,
