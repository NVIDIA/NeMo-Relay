...
`{metadata.atif_prefix:-unassigned}`. Relay also uses the fallback when the
...
placeholder has no fallback and is missing or non-string, or if its resolved
value is unsafe, Relay skips that trajectory. It records a runtime diagnostic
in `plugin.report()` and causes plugin teardown to fail. Literal template text
must also be a relative, traversal-free path. The rendered filename applies to
...
The diagnostic remains visible in `plugin.report()` and is retained there after
a failed teardown. Teardown also reports the degraded delivery, even when the
local recovery write succeeds. Fatal dispatcher failures, such as trajectory
serialization failures, are also reported during teardown.
