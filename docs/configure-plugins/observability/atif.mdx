...
| `header_file` | `{}` | Header names mapped to regular files read for every completed-trajectory upload. Files must exist at activation; trailing whitespace is removed from each value. Remote destinations require HTTPS; HTTP is allowed only for localhost and loopback IPs. Redirects are not followed, so a redirect response fails that upload without forwarding the file-backed value. |
