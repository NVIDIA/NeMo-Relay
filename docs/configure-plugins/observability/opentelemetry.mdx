...
OTLP/HTTP trace exporters do not follow redirects, including redirects to other
HTTPS endpoints. Redirects are disabled to prevent exports from bypassing transport
validation or forwarding payloads and credentials to another destination. Configure
the final collector URL directly. This policy applies to all trace projections; gRPC, log, and metric
...
| `session_filter` | Omitted | Optional endpoint-local trace and log delivery filter. See [Session filtering](#session-filtering). |

## Session Filtering

Trace endpoints and explicit log endpoints can independently stop delivery for
a session after Relay observes a matching tool scope. The matching tool event is
dropped at that endpoint, as are later trace and log events for the same
session. Events exported before the match cannot be retracted, other endpoints
remain independent, and metric delivery is unchanged.
...
The supported filter type is `block_after_tool_match`.
`session_metadata_key` defaults to `session_id`, and
`unattributed_events` defaults to `block_after_match`. Once any session matches,
`block_after_match` also drops events whose session cannot be resolved at that
endpoint. Blocked-session state lasts until the Relay process restarts.

Use the same `session_filter` object on an
`opentelemetry.logs.endpoints[N]` entry to protect an explicit log destination.
When logs are derived from trace endpoints, each log destination inherits its
trace endpoint's filter. Metric endpoints reject `session_filter`.
