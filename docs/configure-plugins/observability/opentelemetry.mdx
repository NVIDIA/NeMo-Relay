...
You can repeat a type or combine types. Each endpoint owns an independent
exporter, tracer provider, batch queue, and runtime. NeMo Relay has no hard
endpoint limit; resource use and egress scale with endpoint count, so validate
large fan-outs against deployment limits.
...
| `max_queue_size` | Omitted | Positive per-endpoint span queue capacity. When omitted, the OpenTelemetry SDK default or `OTEL_BSP_MAX_QUEUE_SIZE` applies. |
