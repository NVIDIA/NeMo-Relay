...
`gen_ai.input.messages` and `gen_ai.output.messages` attributes as JSON strings
that follow the OpenTelemetry GenAI message schemas. The input attribute is
emitted only when the normalized request contains messages. The output
attribute is emitted only when the normalized response contains content parts
or tool calls. Redact sensitive content with an LLM or event sanitizer. Tool
and retrieval payloads are not exported.
