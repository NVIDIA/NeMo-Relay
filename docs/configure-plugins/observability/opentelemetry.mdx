...
| `endpoint` | Required | Nonblank OTLP endpoint. For OTLP/HTTP, Relay appends `/v1/traces` when the endpoint contains only a scheme, host, and optional port, with or without a root trailing `/`. Any other explicit path is preserved. gRPC endpoints are always preserved. |
