...
harness executable version from Relay's existing compatibility probe. Relay
adds `agent_version` alongside `agent_kind` in session scope and LLM metadata,
and on tool identity metadata. This is the harness version, not Relay's version
or the model version. It is omitted for unwrapped sessions and dry runs; hook
or request metadata cannot override the detected value. Metadata remains
subject to the configured sanitizers. To export this generic field as a span
attribute, include `"agent_version"` in `promote_metadata_prefixes`.
