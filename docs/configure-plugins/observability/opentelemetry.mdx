...
For an imported Relay propagation context with both a root and an external
parent, Relay uses the root UUID as the OTLP trace ID and the lower 64 bits of
the parent UUID as a remote parent span ID. A rootless import begins a new
native trace. Relay does not derive W3C sampling flags or `tracestate`; carry
those separately in an integration when they are required.
