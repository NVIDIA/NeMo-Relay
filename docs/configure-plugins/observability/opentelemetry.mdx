...
This TTL applies only to trace endpoints. OTLP logs preserve parent context from
a separate bounded cache of at most 4,096 completed scopes, and metrics do not
retain completed-scope parent context. Do not rely on a closed scope handle for
long-running log follow-up work; emit the mark before closing the scope or use
an active/new scope instead.
