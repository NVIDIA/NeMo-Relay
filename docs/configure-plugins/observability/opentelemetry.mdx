...
OTLP/HTTP trace exporters do not follow redirects, even to another HTTPS
endpoint. This keeps an export from bypassing transport checks or sending its
payload and credentials to another destination. Use the final collector URL.
Log and metric HTTP exporters do the same when any header source is configured.
gRPC does not use HTTP redirects.
