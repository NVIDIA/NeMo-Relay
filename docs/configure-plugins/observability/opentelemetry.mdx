...
Log and metric HTTP exporters do the same when `header_file` is configured.
...
Remote trace endpoints must use HTTPS. When `header_file` is configured, remote
log and metric endpoints must also use HTTPS. HTTP is allowed only for localhost
and loopback IP addresses. OTLP/HTTP exporters do not follow redirects when they
add file-backed values. For example, an OIDC token-file writer must write the
complete value `Bearer eyJ...` to the file. Relay does not add the authentication
scheme for you.
