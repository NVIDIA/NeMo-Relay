...
| `endpoint` | Required | Nonblank OTLP endpoint. For OTLP/HTTP, a URL with no path receives `/v1/traces`; an explicit path is preserved. gRPC endpoints are always preserved. |
