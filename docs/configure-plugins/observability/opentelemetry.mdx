...
registering subscribers. An invalid endpoint prevents activation, and a
delivery failure from one exporter does not stop application work or delivery
...
NeMo Relay does not impose a maximum number of OpenTelemetry destinations.
Each trace, log, or metric destination owns an exporter, signal provider,
processor or reader, and exporter runtime resources. Nothing in those export
stacks is shared between destinations. Queue capacity and memory are per
destination, and total export traffic grows with the destination count.
...
context. Relay leaves OTLP `event_name` unset with OpenTelemetry SDK `0.32` and
retains the dynamic name in `nemo_relay.mark.name`.
...
OTLP conversion. Metric names must be 1–255 ASCII bytes, start with a letter,
...
Metric points use SDK collection timestamps. SDK `0.32` cannot preserve the
source mark timestamp or attach a trace-linked exemplar through this path.
Relay does not emulate correlation with high-cardinality attributes.

### Emit Log and Metric Marks

Rust, Python, and Node.js generic mark APIs accept optional `data_schema` and
`severity` values. Prefer the typed metric helper—`metric` in Rust, Python, and
Node.js, or `EmitMetric` in Go—over constructing the reserved schema by hand.
The helper validates the complete measurement group before publishing the
mark. The exporter validates the sanitized payload again before recording any
measurement.

The following Python example emits one warning log mark and one metric mark:
...
the corresponding standard signal path. Python, Node.js, Go, and C FFI expose
the same three independently managed subscriber kinds; Go and C FFI remain
...
`runtimeDiagnostics()` in Node.js, and `RuntimeDiagnostics()` in Go. It reports
each runtime condition's stable code, occurrence count, and most recent
message. Use it to monitor rejected metric marks, capacity limits, and delivery
failures without configuring the observability plugin. The plugin continues to
include the same conditions in its runtime report.

## Version 3 to Version 4

Version 4 adds the sibling `opentelemetry.logs` and
`opentelemetry.metrics` sections. It is the generated and programmatic default.
Version 3 remains valid for trace-only configuration and rejects either new
signal section, even when that section is disabled. Trace endpoint behavior
and projection controls are unchanged.

To upgrade a version-3 configuration, change `version = 3` to `version = 4`,
then add either signal section only when that export is needed. Omitting the
sections preserves trace-only behavior.
