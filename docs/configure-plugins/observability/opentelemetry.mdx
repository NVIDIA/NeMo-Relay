...
Log lineage follows the same root selection as traces. Relay captures parent
trace context when it queues an event, so a subscriber registered after a
local parent starts can still place later children in the parent's trace. When
neither the parent lifecycle nor captured context is available, the observed
scope starts a trace using its own UUID; a propagation-root UUID alone does not
establish an imported parent. Marks under that scope, including late marks
within the completed-context retention window, use its trace identity. An
explicit imported parent continues the propagated trace.
