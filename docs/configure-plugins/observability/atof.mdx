...
| `header_file` | `{}` | Maps each header name to a regular file containing its full value. Files must exist at activation. HTTP POST reads it per event; WebSocket reads it when connecting or reconnecting; NDJSON reads it when its stream starts. Remote destinations require HTTPS or WSS; HTTP or WS is allowed only for localhost and loopback IPs. |
...
Use `header_file` for credentials that another process rotates in place. The
file content is read as UTF-8 at the transport point noted above, with trailing
whitespace removed. A missing, unreadable, blank, or invalid value fails that
delivery without logging the value. A header may appear in only one of
`headers`, `header_env`, and `header_file`, ignoring ASCII case.

When `header_file` is configured, ATOF HTTP clients do not follow redirects.
Configure the final collector URL directly; a redirect response fails that
delivery without forwarding the file-backed value.
