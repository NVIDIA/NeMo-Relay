...
Each file or stream sink can include an `activation_policy` table. Denying a
file sink prevents Relay from opening its file. If every sink is denied, Relay
does not install an ATOF exporter; canonical event generation and unrelated
subscribers continue normally.
