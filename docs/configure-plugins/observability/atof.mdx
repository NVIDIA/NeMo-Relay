...
| `header_file` | `{}` | Maps each header name to a file containing its full value. Files must exist at activation. HTTP POST reads it per event; WebSocket reads it when connecting or reconnecting; NDJSON reads it when its stream starts. |
