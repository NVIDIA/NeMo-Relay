...
| `header_file` | `{}` | Maps each header name to a regular file that holds its full value. Files must exist at activation. HTTP POST reads it for each event. WebSocket reads it when it connects or reconnects. NDJSON reads it when its stream starts. Remote destinations must use HTTPS or WSS. HTTP or WS is allowed only for localhost and loopback IPs. |
...
When `header_file` is configured, ATOF HTTP clients do not follow redirects.
Use the final collector URL. If a server sends a redirect, that delivery fails
and Relay does not send the file-backed value to the new destination.
