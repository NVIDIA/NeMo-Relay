...
| `header_file` | `{}` | Maps each header name to a regular file that holds its full value. Files must exist at activation. HTTP POST reads it for each event. WebSocket reads it when it connects or reconnects. NDJSON reads it when its stream starts. |
...
Use `header_file` when another process updates a credential in place. Relay
reads the UTF-8 file at the time listed above and removes trailing whitespace.
If the file is missing, unreadable, blank, or invalid, only that delivery fails.
Relay does not log the value. A header can use only one of `headers`,
`header_env`, and `header_file`. Header names are case-insensitive.

When you set any header source (`headers`, `header_env`, or `header_file`),
remote ATOF HTTP destinations must use HTTPS and remote WebSocket destinations
must use WSS. HTTP or WS is allowed only for localhost and loopback IP
addresses.

ATOF HTTP clients do not follow redirects when headers are set. Use the final
collector URL. If a server sends a redirect, that delivery fails and Relay does
not send headers to the new destination.
...
During graceful shutdown, `activation.close()` uses that same terminal
stream-sink operation. Await it from Python and Node.js so queued sanitizers
can finish on the running event loop. Teardown does not guarantee that a worker
which timed out drained or closed.

## Migrate From Version 1

Use observability component `version = 4`. Version 3 remains accepted for
trace-only compatibility and uses the same typed ATOF sink shape. NeMo Relay
