...
| Generic OTLP traces or opt-in OpenTelemetry GenAI 1.37+ spans | [OpenTelemetry](/configure-plugins/observability/opentelemetry) |
...
For coding-agent sessions, OTLP trace roots additionally carry the logical
`session.id`, optional `user.id`, and Relay-owned
...
OpenTelemetry and OpenInference generate independent native trace IDs, so use
the Relay instance ID for cross-exporter or cross-trace correlation and use
each backend's `trace_id` to join child rows. ATIF keeps its existing artifact
IDs and exposes the same logical values under `extra.nemo_relay`.
...
- [Agent Trajectory Interchange Format (ATIF)](/configure-plugins/observability/atif) covers
  ...
  scopes.
- [OpenTelemetry](/configure-plugins/observability/opentelemetry) covers generic OTLP trace export and the opt-in GenAI semantic projection.
