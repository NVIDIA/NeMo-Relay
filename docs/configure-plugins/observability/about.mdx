...
so the backend IDs also correlate across exporters. ATIF artifact filenames and
storage keys remain based on the trajectory scope UUID. Logical coding-session
values also remain available under `extra.nemo_relay`.
