...
| `mark_projection` | `inherit` | `inherit` uses exporter-native handling; `event` forces span events; `tool` emits zero-duration OpenInference `TOOL` child spans. |
...
The default `inherit` projection preserves exporter-native mark handling. With
`mark_projection = "tool"`, each non-streaming mark becomes a zero-duration child span with
`openinference.span.kind = "TOOL"` and the original mark payload, category,
profile, timestamp, and parent identifiers retained as attributes. High-volume
`llm.chunk` marks remain span events.

Add other event names to `mark_exclude_names` when a backend should keep those
marks as native span events rather than visible tool children. The exclusion
