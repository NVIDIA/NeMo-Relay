...
| `mark_projection` | `event` | `event` attaches marks to active parent spans. `tool` emits zero-duration OpenInference `TOOL` child spans. |
...
The default `event` projection preserves marks as span events. With
