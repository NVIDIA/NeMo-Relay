...
| `opentelemetry.endpoints` | Fans out trace events to typed `full`, `gen_ai`, or `openinference` OTLP endpoints. |
| `opentelemetry.logs` | Exports sanitized non-metric marks as OTLP logs. |
| `opentelemetry.metrics` | Records Relay metric-schema marks and exports OTLP metrics. |
...
logs := nemo_relay.NewObservabilityOpenTelemetryLogConfig()
logs.Enabled = true
metrics := nemo_relay.NewObservabilityOpenTelemetryMetricConfig()
