...
trace destination, so a bare URL (with or without a root trailing `/`) and the
same URL with `/v1/traces` also collide. The comparison realizes HTTP port `80`
and HTTPS port `443`, collapses repeated path slashes, ignores a non-root
trailing slash, and treats standardized
