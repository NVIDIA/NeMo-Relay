...
## Export Activation Policies

Version 4 export targets accept an optional `activation_policy`. The
`provider` is the ordinary plugin kind or dynamic-plugin manifest ID of a
component that registers one policy callback. Dynamic providers also declare
the `export_activation_policy` capability:

```toml
[components.config.opentelemetry.endpoints.activation_policy]
provider = "com.example.runtime-policy"
timeout_millis = 30000
config = { allowed_countries = ["US", "CA"], min_available_memory_mib = 2048 }
```

Relay evaluates the policy once for each configured target before constructing
that exporter. The callback receives only the target kind and the opaque
`config` value. Relay does not send the destination URL, headers, credentials,
or other exporter settings to the policy. The decision remains fixed until
plugin reconfiguration or restart.

Configuration layering finishes before policy providers register. Each
resolved plugin activation owns one provider namespace, and every target in
that activation resolves its provider manifest ID against that namespace.
Provider implementations do not shadow one another by configuration layer;
layered targets can reference the same provider with different opaque `config`
values. A later activation receives a new provider namespace.

Policies are available on OTLP trace, log, and metric endpoints; ATOF stream
sinks; and ATIF HTTP and S3 targets. Local ATOF and ATIF file destinations do
not accept policies. Derived OTLP log and metric endpoints inherit the trace
policy, but Relay evaluates each signal separately with `otlp_log` or
`otlp_metric` as its target kind.

The timeout defaults to 30000 milliseconds and must be between 1000 and 300000
(one second and five minutes). Relay enforces these bounds when evaluating a
policy even when the configuration policy reports unsupported values as
warnings.
A deny decision, missing provider, callback error, unsupported worker surface,
or timeout denies only that target. Relay continues activation and emits a
structured diagnostic that contains the provider ID, target kind, and reason,
but no destination or credential data. A denied target is never constructed,
connected, or given a background exporter task.

Mixed ATOF configurations retain allowed local sinks. When policy evaluation
denies every remote ATIF target, Relay disables the ATIF dispatcher and does
not silently fall back to its legacy local-file destination. Policies are an
activation-time control only; per-event reevaluation is not supported.
  ...
  opentelemetry: observability.openTelemetryConfig({
    ...
    endpoints: [
      observability.openTelemetryEndpoint({
        ...
        endpoint: "http://localhost:4318/v1/traces",
        service_name: "agent-service",
        activation_policy: observability.exportActivationPolicy({
          provider: "com.example.runtime-policy",
          config: { allowed_countries: ["US", "CA"] },
        }),
      }),
