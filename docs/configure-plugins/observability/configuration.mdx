...
```go
import nemo "github.com/NVIDIA/NeMo-Relay/go/nemo_relay"
...
logs := nemo.NewObservabilityOpenTelemetryLogConfig()
logs.Enabled = true
metrics := nemo.NewObservabilityOpenTelemetryMetricConfig()
metrics.Enabled = true
otel.Logs = &logs
otel.Metrics = &metrics
config.OpenTelemetry = &otel
config.EnableFullPayloads = true
component := nemo.NewObservabilityComponentSpec(config)
```
</Tab>
</Tabs>
