...
Version 4 remote targets accept an optional `activation_policy`. The
`provider` is the dynamic-plugin manifest ID of a native or worker plugin that
declares the `export_activation_policy` capability and registers one callback:
