...
rule, so system, project, user, and programmatic layers can contribute
destinations. Arbitrary lists nested inside structured values retain
replacement semantics. List entries are not merged item by item.
To change or remove an inherited endpoint, edit the layer that declares it.
...
`semantic_selector`, and `capture_content`. Their behavior is fixed by the
selected projection. Version-2 OTLP shapes are rejected under version 3.
