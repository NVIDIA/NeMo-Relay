...
The plugin does not start or supervise the Switchyard service. Relay does not
require the service to be healthy at startup; unavailable Decision API calls
fail open to the configured same-protocol fallback. Profiles that depend on
ATOF history, including StageRouter, must use
`context_mode = "atof_required"` and name an enabled Relay ATOF HTTP endpoint
with `atof_endpoint_name`. Relay fails startup validation when that endpoint is
missing or invalid. Relay pushes ATOF events to the endpoint; local ATOF JSONL
output alone does not populate Switchyard's accumulator.
...
- Enforce or observe-only routing decisions.
