...
| `priority` | `0` | Component execution priority. When response caching is enabled, this value must be lower than `response_cache.priority` so Switchyard selects the backend before Relay derives the cache key. |
