...
> running Switchyard Decision API. The in-process `libsy` design is part of the
> NeMo Relay 0.7 work and does not apply to NeMo Relay 0.6.0.
...
- The Decision API and service/library boundary are experimental. The
  in-process Decision Provider work targets NeMo Relay 0.7 and is not part of
  NeMo Relay 0.6.0.
