...
The unauthenticated `/healthz` and `/_nemo_relay/v1/workers` operational paths
are reachable anywhere the daemon listener is reachable. They expose no
credentials, user fingerprints, or worker endpoints, but worker UUIDs and
activity counts are operational metadata. Limit the listener, firewall, and any
reverse proxy to the approved client and operations networks.
