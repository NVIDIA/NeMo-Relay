...
### Inspect Route and Worker Status

The daemon exposes its current broker route snapshots for deployment diagnostics:

```bash
curl -f http://127.0.0.1:47632/_nemo-relay/v1/route/status
```

The response reports each route's lifecycle state, route mode, MCP reference
count, pass-through kind, and last-known worker ID, control availability, and
in-flight request count when a worker is assigned. It never includes route
credentials, fingerprints, worker endpoints, session IDs, activation IDs, or
internal worker credentials. The snapshots are instantaneous broker state, not
active worker liveness probes.

<Warning>
  This endpoint is temporarily unauthenticated. Bind the daemon to a trusted
  interface and restrict network access until administrator authentication is
  added.
</Warning>
