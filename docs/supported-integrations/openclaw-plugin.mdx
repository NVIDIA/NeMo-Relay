...
- `config.plugins.components[].config.opentelemetry` sends OTLP spans to an
  OpenTelemetry collector when `enabled` is `true`. Generic Relay semantics are
  the default; set `semantic_convention` to `gen_ai` for the OpenTelemetry GenAI
  1.37 or newer projection.
