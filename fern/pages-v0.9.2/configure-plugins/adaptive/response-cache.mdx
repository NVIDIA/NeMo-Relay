...
- Stateful requests bypass the cache entirely: a request that opts into
  ...
  cannot see.
- Anthropic Messages requests that use server-side compaction, carry a prior
  compaction block, or send a compaction beta header run live. Relay does not
  cache them because compaction changes continuation and streaming behavior
  that response-cache replay does not yet preserve.
...
Other bypass reasons include `anthropic_compaction`, `sampled`,
`stateful_store`, and `stream_no_codec`. A failed cache-store operation
produces a `miss` with `reason = "store_error"`.
