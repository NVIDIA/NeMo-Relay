...
or `opaque`. Every callback declares `(payload, context)` and may omit the LLM
event payload by returning no value. In-process sanitizer contexts can resolve
the active codec for normalized processing. Worker sanitizer contexts resolve
an invocation-scoped asynchronous codec proxy, so workers can perform the same
directional normalization without receiving a host-process object.
