...
configuration. An explicit editor scope flag overrides this inherited target.
These rules only select the file opened by the editor; they do not change
runtime discovery, layering, or merge precedence.
...
layer. Other nested lists replace. For example, the following system
...
The user scope can add another sink without repeating the system sink:
...
`version` and `enabled` from the system file. Its `sinks` list contains the user
sink first, followed by the system sink.
...
falling back to fleet-managed model pricing from
