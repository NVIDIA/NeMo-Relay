...
Query the daemon's aggregate worker status while the harness remains open:

```bash
curl -f http://127.0.0.1:47632/_nemo_relay/v1/workers/status
```

Confirm that the expected worker UUID appears with `state = "ready"` and
`control_available = true`. Use
`/_nemo_relay/v1/workers/<instance_uuid>/status` to inspect that worker alone.
...
An empty worker list also does not prove a failure. Global pass-through,
transient fail-open, and activation-in-progress routes have no assigned worker
UUID and are omitted. Compare `/healthz`, worker status, lifecycle events, and an
observable plugin result before deciding that traffic used a worker.
...
| Worker status list is empty | No assigned worker, activation still pending, or route is in pass-through | Check `/healthz` deployment mode and lifecycle events; keep the harness open while verifying. |
| Worker status returns `404` | The UUID is unknown, drained, replaced, or no longer assigned | Refresh `/_nemo_relay/v1/workers`; correlate the replacement UUID with lifecycle logs. |
| Worker reports `control_available = false` | Worker control disconnected and may be within recovery grace | Check worker control events and wait up to the documented recovery window before restarting the harness. |
