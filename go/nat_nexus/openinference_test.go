...
func TestNewOpenInferenceConfigDefaults(t *testing.T) {
	...
	if config.ServiceName != "nat-nexus" {
		t.Fatalf("expected default service name nat-nexus, got %q", config.ServiceName)
	}
	if config.InstrumentationScope != "nvidia-nat-nexus-openinference" {
