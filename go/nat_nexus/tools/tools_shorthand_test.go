...
import (
	...
	toolspkg "gitlab-master.nvidia.com/nemo-agent-toolkit/dev/Project-NAT-Nexus/go/nat_nexus/tools"
)
...
func TestToolShorthands(t *testing.T) {
	...
	if err := nat_nexus.RegisterToolRequestIntercept("tools_req_int", 1, false,
	...
	t.Cleanup(func() {
		_ = nat_nexus.DeregisterToolRequestIntercept("tools_req_int")
	})
	...
	if err := nat_nexus.RegisterToolConditionalExecutionGuardrail("tools_cond", 1,
		func(name string, args json.RawMessage) *string { return nil },
	); err != nil {
		t.Fatalf("RegisterToolConditionalExecutionGuardrail failed: %v", err)
	}
	t.Cleanup(func() {
		_ = nat_nexus.DeregisterToolConditionalExecutionGuardrail("tools_cond")
	})
