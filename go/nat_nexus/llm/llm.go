...
// Package llm provides shorthand access to Nexus LLM call operations.
//
...
// Example usage:
//
//	import "gitlab-master.nvidia.com/nemo-agent-toolkit/dev/Project-NAT-Nexus/go/nat_nexus/llm"
//
...
// Call starts an LLM call lifecycle and returns an [nat_nexus.LLMHandle],
// emitting a Start event. End the call with [CallEnd]. This is a shorthand for
// [nat_nexus.LlmCall].
func Call(name string, native interface{}, opts ...nat_nexus.LLMCallOption) (*nat_nexus.LLMHandle, error) {
	return nat_nexus.LlmCall(name, native, opts...)
}

// CallEnd completes an LLM call that was started with [Call], emitting an End
// event. This is a shorthand for [nat_nexus.LlmCallEnd].
func CallEnd(handle *nat_nexus.LLMHandle, response json.RawMessage, opts ...nat_nexus.LLMCallOption) error {
	return nat_nexus.LlmCallEnd(handle, response, opts...)
}
...
func Execute(name string, native interface{}, fn nat_nexus.LLMExecutionFunc, opts ...nat_nexus.LLMCallOption) (json.RawMessage, error) {
	return nat_nexus.LlmCallExecute(name, native, fn, opts...)
}
...
// returns an [nat_nexus.LlmStream] for consuming JSON chunks. This is a
// shorthand for [nat_nexus.LlmStreamCallExecute].
//
...
func StreamExecute(name string, native interface{}, fn nat_nexus.LLMExecutionFunc, collector nat_nexus.CollectorFunc, finalizer nat_nexus.FinalizerFunc, opts ...nat_nexus.LLMCallOption) (*nat_nexus.LlmStream, error) {
	return nat_nexus.LlmStreamCallExecute(name, native, fn, collector, finalizer, opts...)
}
...
// [nat_nexus.LlmConditionalExecution].
