...
func TestScopeShorthands(t *testing.T) {
	...
	if err := nemo_flow.RegisterSubscriber("scope_shortcuts_sub", func(event nemo_flow.Event) {
	...
	defer nemo_flow.DeregisterSubscriber("scope_shortcuts_sub")

	handle, err := scope.Push("scope-shortcuts", nemo_flow.ScopeTypeFunction)
	if err != nil {
