...
import (
	...
	llmpkg "github.com/NVIDIA/NeMo-Flow/go/nemo_flow/llm"
)
...
func TestLlmShorthands(t *testing.T) {
	...
	if err := nemo_flow.RegisterLlmRequestIntercept("llm_req_int", 1, false,
	...
	t.Cleanup(func() {
		_ = nemo_flow.DeregisterLlmRequestIntercept("llm_req_int")
	})
	...
	if err := nemo_flow.RegisterLlmConditionalExecutionGuardrail("llm_cond", 1,
		func(headers, content json.RawMessage) *string { return nil },
	); err != nil {
		t.Fatalf("RegisterLlmConditionalExecutionGuardrail failed: %v", err)
	}
	t.Cleanup(func() {
		_ = nemo_flow.DeregisterLlmConditionalExecutionGuardrail("llm_cond")
	})
