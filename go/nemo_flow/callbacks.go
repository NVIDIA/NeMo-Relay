...
// callbacks.go defines the Go callback type aliases used by the NeMo Flow
...
static inline char* callToolExecNext(NemoFlowToolExecNextFn next_fn, const char* args_json, void* next_ctx) {
	return next_fn(args_json, next_ctx);
}

// Helper to call the LLM exec next function pointer from Go
static inline char* callLlmExecNext(NemoFlowLlmExecNextFn next_fn, const char* native_json, void* next_ctx) {
...
typedef char* (*NemoFlowCodecDecodeCb)(void* user_data, const FfiLLMRequest* request);
typedef char* (*NemoFlowCodecEncodeCb)(void* user_data, const char* annotated_json, const FfiLLMRequest* original_request);
typedef NemoFlowCodecDecodeCb NemoFlowCodecDecodeFn;
typedef NemoFlowCodecEncodeCb NemoFlowCodecEncodeFn;
*/
import "C"
...
func setLastErrorMessage(msg string) {
	...
	C.nemo_flow_set_last_error_message(cMsg)
}
...
func registerClosure(fn interface{}) unsafe.Pointer {
	...
	if p == nil {
		panic("nemo_flow: failed to allocate callback token")
	}
	*p = id
...
func goLlmRequestTrampoline(userData unsafe.Pointer, request *C.FfiLLMRequest) *C.FfiLLMRequest {
	...
	cHeaders := C.nemo_flow_llm_request_headers(request)
	cContent := C.nemo_flow_llm_request_content(request)
	goHeaders := json.RawMessage(C.GoString(cHeaders))
	goContent := json.RawMessage(C.GoString(cContent))
	C.nemo_flow_string_free(cHeaders)
	C.nemo_flow_string_free(cContent)
	...
	return C.nemo_flow_llm_request_new(cNewHeaders, cNewContent)
}
...
func goToolExecInterceptTrampoline(userData unsafe.Pointer, argsJSON *C.char, nextFn C.NemoFlowToolExecNextFn, nextCtx unsafe.Pointer) *C.char {
	...
	goNext := func(args json.RawMessage) (json.RawMessage, error) {
		...
		defer C.nemo_flow_string_free(result)
...
func goLlmExecInterceptTrampoline(userData unsafe.Pointer, nativeJSON *C.char, nextFn C.NemoFlowLlmExecNextFn, nextCtx unsafe.Pointer) *C.char {
...
func goCodecEncodeTrampoline(userData unsafe.Pointer, annotatedJSON *C.char, originalRequest *C.FfiLLMRequest) *C.char {
	...
	cOrigHeaders := C.nemo_flow_llm_request_headers(originalRequest)
	cOrigContent := C.nemo_flow_llm_request_content(originalRequest)
	goOrigHeaders := json.RawMessage(C.GoString(cOrigHeaders))
	goOrigContent := json.RawMessage(C.GoString(cOrigContent))
	C.nemo_flow_string_free(cOrigHeaders)
	C.nemo_flow_string_free(cOrigContent)
...
func goLlmRequestInterceptTrampoline(
	...
	if err != nil {
		setLastErrorMessage(err.Error())
		return 5 // NemoFlowStatus::Internal
	}
	...
	defer C.free(unsafe.Pointer(cNewHeaders))
	defer C.free(unsafe.Pointer(cNewContent))
	*outRequest = C.nemo_flow_llm_request_new(cNewHeaders, cNewContent)
	if newAnnotated != nil {
