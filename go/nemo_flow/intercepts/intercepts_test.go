...
import (
	...
	"github.com/NVIDIA/NeMo-Flow/go/nemo_flow/intercepts"
)
...
func TestInterceptShorthandsGlobal(t *testing.T) {
	...
	result, err := nemo_flow.ToolCallExecute("intercepts_tool", json.RawMessage(`{"value": 1}`),
	...
	response, err := nemo_flow.LlmCallExecute("intercepts_llm", map[string]interface{}{
...
func TestInterceptShorthandsScopeLocal(t *testing.T) {
	...
	stack.Run(func() {
		handle, err := nemo_flow.PushScope("intercepts_scope", nemo_flow.ScopeTypeAgent)
		if err != nil {
		...
		if _, err := nemo_flow.ToolCallExecute("intercepts_scope_tool", json.RawMessage(`{"ok": true}`),
		...
		if _, err := nemo_flow.LlmCallExecute("intercepts_scope_llm", map[string]interface{}{
			"headers": map[string]interface{}{},
			"content": map[string]interface{}{"messages": []interface{}{}, "model": "test-model"},
		}, func(nativeJSON json.RawMessage) (json.RawMessage, error) {
			return json.RawMessage(`{"ok": true}`), nil
