...
import (
	...
	"github.com/NVIDIA/NeMo-Flow/go/nemo_flow/guardrails"
)
...
func TestGuardrailShorthandsGlobal(t *testing.T) {
	...
	if err := nemo_flow.RegisterSubscriber("guardrails_events", func(event nemo_flow.Event) {
		mu.Lock()
		defer mu.Unlock()
	...
	t.Cleanup(func() {
		_ = nemo_flow.DeregisterSubscriber("guardrails_events")
	})
	...
	_, err := nemo_flow.ToolCallExecute("guardrails_tool", json.RawMessage(`{"value": 1}`),
	...
	_, err = nemo_flow.LlmCallExecute("guardrails_llm", makeRequest(),
...
func TestGuardrailShorthandsScopeLocal(t *testing.T) {
	...
	stack.Run(func() {
		handle, err := nemo_flow.PushScope("guardrails_scope", nemo_flow.ScopeTypeAgent)
		if err != nil {
		...
		if _, err := nemo_flow.ToolCallExecute("guardrails_scope_tool", json.RawMessage(`{"ok": true}`),
		...
		if _, err := nemo_flow.LlmCallExecute("guardrails_scope_llm", makeRequest(),
			func(nativeJSON json.RawMessage) (json.RawMessage, error) {
				return json.RawMessage(`{"ok": true}`), nil
			},
		); err != nil {
