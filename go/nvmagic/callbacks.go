...
// callbacks.go defines the Go callback type aliases used by the NVMagic
...
static inline char* callToolExecNext(NvMagicToolExecNextFn next_fn, const char* args_json, void* next_ctx) {
	return next_fn(args_json, next_ctx);
}

// Helper to call the LLM exec next function pointer from Go
static inline char* callLlmExecNext(NvMagicLlmExecNextFn next_fn, const char* native_json, void* next_ctx) {
...
func goLlmRequestTrampoline(userData unsafe.Pointer, request *C.FfiLLMRequest) *C.FfiLLMRequest {
	...
	cHeaders := C.nvmagic_llm_request_headers(request)
	cContent := C.nvmagic_llm_request_content(request)
	goHeaders := json.RawMessage(C.GoString(cHeaders))
	goContent := json.RawMessage(C.GoString(cContent))
	C.nvmagic_string_free(cHeaders)
	C.nvmagic_string_free(cContent)
	...
	return C.nvmagic_llm_request_new(cNewHeaders, cNewContent)
}
...
func goToolExecInterceptTrampoline(userData unsafe.Pointer, argsJSON *C.char, nextFn C.NvMagicToolExecNextFn, nextCtx unsafe.Pointer) *C.char {
	...
	goNext := func(args json.RawMessage) (json.RawMessage, error) {
		...
		defer C.nvmagic_string_free(result)
...
func goLlmExecInterceptTrampoline(userData unsafe.Pointer, nativeJSON *C.char, nextFn C.NvMagicLlmExecNextFn, nextCtx unsafe.Pointer) *C.char {
