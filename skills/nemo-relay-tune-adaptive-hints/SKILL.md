---
name: nemo-relay-tune-adaptive-hints
description: Consume NeMo Relay adaptive hints, predictions, latency sensitivity, ACG diagnostics, or tool-parallelism guidance safely in application logic after the adaptive plugin is already configured
...
Use this skill when the adaptive plugin is already configured and the
application wants to consume adaptive outputs such as model-request hints,
parallelism guidance, or ACG diagnostics.

## Do Not Use This When

Do not use this skill to design the first adaptive rollout or to configure the
plugin from scratch. Use `nemo-relay-tune-performance` or
`nemo-relay-tune-adaptive-config` first.
