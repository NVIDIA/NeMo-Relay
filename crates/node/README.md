...
before the call without blocking the Node.js event loop. JavaScript subscribers
run later through Node's callback queue, so native events they emit are separate
publications. The extra event-loop turn lets queued JavaScript callback side
effects complete before deregistration or exit; flush again if those side
effects emit native events that must also be observed.
