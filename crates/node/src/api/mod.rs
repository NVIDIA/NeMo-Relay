...
pub fn pop_scope(handle: &JsScopeHandle) -> Result<()> {
    core_scope_api::pop_scope(&handle.inner.uuid).map_err(to_napi_err)?;
...
pub fn with_scope(
    ...
    let pa_fn = std::sync::Arc::new(
        crate::promise_call::PromiseAwareFn::new(&env, &callback).map_err(|e| {
            ...
            let _ = core_scope_api::pop_scope(&scope_uuid);
        ...
        async move {
            TASK_SCOPE_STACK
                .scope(scope_stack, async move {
                    ...
                    if core_scope_api::pop_scope(&scope_uuid).is_ok() {
...
pub fn tool_call_end(
    ...
    core_tool_api::tool_call_end(&handle.inner, result, opt_json(data), opt_json(metadata))
        .map_err(to_napi_err)
}
