...
impl From<ScopeStackHandle> for JsScopeStack {
...
impl From<ScopeHandle> for JsScopeHandle {
...
impl From<ToolHandle> for JsToolHandle {
...
impl From<LLMHandle> for JsLLMHandle {
...
#[napi]
pub const SCOPE_ATTR_PARALLEL: u32 = ScopeAttributes::PARALLEL.bits();
/// Scope attribute flag: the scope can be relocated to a different parent.
#[napi]
pub const SCOPE_ATTR_RELOCATABLE: u32 = ScopeAttributes::RELOCATABLE.bits();
/// Tool attribute flag: the tool executes locally (not via remote API).
#[napi]
pub const TOOL_ATTR_LOCAL: u32 = ToolAttributes::LOCAL.bits();
/// LLM attribute flag: the LLM call is stateless (no conversation context).
#[napi]
pub const LLM_ATTR_STATELESS: u32 = LLMAttributes::STATELESS.bits();
/// LLM attribute flag: the LLM call uses streaming responses.
#[napi]
pub const LLM_ATTR_STREAMING: u32 = LLMAttributes::STREAMING.bits();
...
impl JsOpenAIChatCodec {
    ...
    pub fn decode(&self, request: Json) -> napi::Result<Json> {
        let llm_req: LLMRequest = serde_json::from_value(request)
    ...
    pub fn encode(&self, annotated: Json, original: Json) -> napi::Result<Json> {
        ...
        let orig: LLMRequest = serde_json::from_value(original)
            .map_err(|e| napi::Error::from_reason(format!("invalid LLMRequest: {e}")))?;
        let result = self
            .inner_codec
            .encode(&ann, &orig)
