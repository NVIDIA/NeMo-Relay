...
function mockEncode({ annotated, original }) {
  ...
  return { headers: original.headers, content };
}
...
describe('Codec pipeline integration', () => {
  it('intercept receives annotated when codec is active', async () => {
    ...
    registerLlmRequestIntercept('codec-pipeline-test', 10, false,
    ...
    try {
      await execWithCodec(
        'test-llm',
        makeRequest(),
        async (req) => ({ choices: [{ message: { content: 'Hi' } }] }),
        mockDecode,
        mockEncode,
      );
  ...
  it('intercept modifications to annotated flow through encode', async () => {
    ...
    registerLlmRequestIntercept('modify-test', 10, false,
      ({ name, request, annotated }) => {
        if (annotated) {
          annotated = { ...annotated, model: 'gpt-4-turbo' };
        }
        return { request, annotated };
      }
    );
  ...
  it('different codec functions produce different results', async () => {
    ...
    const decodeA = (req) => ({ messages: [], model: 'from-a', params: null, tools: null, tool_choice: null, extra: {} });
    const encodeA = ({ annotated, original }) => ({ headers: original.headers, content: { ...original.content, model: annotated.model } });

    const decodeB = (req) => ({ messages: [], model: 'from-b', params: null, tools: null, tool_choice: null, extra: {} });
    const encodeB = ({ annotated, original }) => ({ headers: original.headers, content: { ...original.content, model: annotated.model } });

    registerLlmRequestIntercept('override-test', 10, false,
      ({ name, request, annotated }) => {
