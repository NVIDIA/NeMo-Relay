...
// Codex sends the hook-compatible root session id in Responses client metadata while retaining
// the rollout/thread id as `prompt_cache_key`. Prefer the shared session id so root and subagent
// provider traffic joins the hook session, then keep the prompt-cache fallback for older clients.
// The installation metadata check prevents treating arbitrary application identifiers as Codex
// session ids.
...
// Codex identifies thread-spawned subagent provider calls with `collab_spawn`. The shared
// `session_id` remains the root hook session, while `thread_id` matches the lifecycle `agent_id`.
// Other internal request kinds remain root-session activity rather than gaining unmatched
// subagent ownership.
pub(crate) fn responses_subagent_id(body: &Value, route: GatewayRouteKind) -> Option<String> {
    ...
    let kind = json_string_at(body, &[&["client_metadata", "x-openai-subagent"][..]]);
    (kind.as_deref() == Some("collab_spawn"))
...
#[cfg(test)]
#[path = "../../../tests/coverage/agents/alignment_codex_tests.rs"]
mod tests;
