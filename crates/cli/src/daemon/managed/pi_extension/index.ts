...
export default function managedNemoRelayPi(pi: ExtensionAPI): void {
  ...
  pi.on('session_shutdown', async (event, context) => {
    ...
    if (event.reason === 'quit') {
      await hookQueue;
      // Use only the existing lease: shutdown must not initialize or restart MCP.
      const active = runtimePromise ? await runtimePromise : undefined;
      if (active) {
        try {
          const outcome = await postHook(active, payload(context, 'session_shutdown', { reason: event.reason }));
          if (outcome.kind !== 'allow') {
            console.error(`NeMo Relay managed hook was not accepted: ${outcome.reason}`);
          }
        } finally {
...
function readDeploymentConfig(): DeploymentConfig {
  ...
  const host = daemon.hostname.replace(/^\[|\]$/g, '');
  const loopback = host === 'localhost' || host === '::1' || /^127(?:\.\d{1,3}){3}$/.test(host);
  if (daemon.protocol === 'http:' && !loopback) {
    throw new Error('managed Pi daemonAddress must use HTTPS unless it names a loopback host');
  }
  return {
