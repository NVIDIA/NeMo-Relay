...
/// Owns session accounting across middleware, dispatch, and off-path observation. Cleanup is
/// spawned before it is awaited, so cancellation of either delivery or cleanup cannot lose it.
...
impl GatewayCallCleanup {
    fn new(sessions: SessionManager, prep: &GatewayCallPrep) -> Self {
        Self {
...
impl Drop for GatewayCallCleanup {
    fn drop(&mut self) {
        // Dropping a JoinHandle detaches the cleanup rather than cancelling it.
...
impl ManagedRuntime {
    ...
    async fn proxy_managed(
        ...
        let handle = match handle {
            ...
            Err(error) => {
                cleanup.finish().await;
                return Err(CliError::Flow(error));
            }
        };
...
impl ObservationReceiver {
    ...
    async fn finish_inner(mut self, surface: ProviderSurface, streaming: bool) -> ObservedResponse {
