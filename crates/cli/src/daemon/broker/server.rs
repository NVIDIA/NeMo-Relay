...
async fn recover_worker(
    ...
    .unwrap_or_else(|error| {
        control_error(
            ...
            CliError::Launch(format!("worker recovery task failed: {error}")),
        )
    })
}

fn recover_worker_after_validation(
...
async fn finish_ready_worker(
            ...
            WorkerPublication::Recovery { .. } => {
                ...
                .unwrap_or_else(|error| {
                    log::error!(
                        ...
                        "Worker generation revocation task failed after readiness probe; continuing cleanup"
                    );
                    true
                })
            }
    ...
    .unwrap_or_else(|error| {
        control_error(
            StatusCode::INTERNAL_SERVER_ERROR,
            CliError::Launch(format!("worker readiness publication task failed: {error}")),
        )
    })
}

fn publish_ready_worker(
    state: Arc<DaemonState>,
    fingerprint: Fingerprint,
    target: Arc<WorkerTarget>,
    publication: WorkerPublication,
    ...
    // This serializes the durable generation update with broker publication so restart recovery
    // cannot race a replacement readiness probe. The complete file-backed transaction runs on a
    // blocking thread and this section performs no network I/O.
...
fn is_request_body_failure(error: &(dyn StdError + 'static)) -> bool {
    let mut source = Some(error);
    while let Some(current) = source {
        if current
            .downcast_ref::<hyper::Error>()
            .is_some_and(|error| error.is_user() || error.is_body_write_aborted())
        {
            return true;
        }
    ...
    match action {
        ...
        ReleaseAction::BeginDrain {
            ...
            tokio::spawn(async move {
                ...
                if let Err(error) = tokio::task::spawn_blocking(move || {
                    revoke_worker_generation(&revoke_state, &worker_id);
                })
                .await
                {
                    log::error!(
                        ...
                        event = "worker_generation_revocation_join_failed",
                        error_kind = if error.is_panic() { "panic" } else { "cancelled" };
                        "Worker generation revocation task failed during drain; continuing cleanup"
                    );
                }
...
fn revoke_worker_generation(state: &DaemonState, worker_id: &str) {
    let generation = worker_generation(state, worker_id);
    if let Some((fingerprint, generation_id)) = generation {
        revoke_active_worker_generation(state, fingerprint, &generation_id);
    }
}

fn worker_generation(state: &DaemonState, worker_id: &str) -> Option<(Fingerprint, String)> {
    // End the session-lock scope before callers acquire the publication lock.
...
fn daemon_origin(options: &ServerOptions, local: SocketAddr) -> Result<String, CliError> {
    ...
    // Signed transcript origins use URL canonicalization, unlike process argv which retains
    // explicit ports. Match the client even when the listener uses HTTP :80 or HTTPS :443.
    Ok(daemon_url(&origin)?
        .as_str()
        .trim_end_matches('/')
        .to_owned())
}
            ...
            accepted = listener.accept() => {
                let (stream, peer) = match accepted {
                    ...
                    Err(error) => {
                        log::warn!(
                            target: "nemo_relay.daemon",
                            event = "tls_listener_accept_failed";
                            "TLS listener accept failed: {error}"
                        );
