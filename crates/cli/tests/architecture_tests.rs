...
impl<'ast> Visit<'ast> for StreamingSourceVisitor {
    ...
    fn visit_expr_method_call(&mut self, call: &'ast syn::ExprMethodCall) {
        ...
        match method.as_str() {
            ...
            "collect" if self.await_depth > 0 => self.reject(".collect()"),
...
fn daemon_streaming_modules_do_not_use_response_aggregation_apis() {
    ...
    let streaming_modules = rust_files(&daemon)
        .into_iter()
        .filter(|path| {
            let source = fs::read_to_string(path).unwrap();
            ["RelayBody", "Frame<Bytes>", "Response<Body>"]
                .iter()
                .any(|streaming_type| source.contains(streaming_type))
        })
        .collect::<Vec<_>>();
    assert!(
        !streaming_modules.is_empty(),
        "no daemon streaming modules discovered"
    );
    for path in streaming_modules {
        let relative = path
            .strip_prefix(&src)
            .unwrap()
            .components()
...
fn streaming_source_analysis_detects_formatted_aliased_and_manual_aggregation() {
    ...
    for expected in [
        "alias for a forbidden aggregation API",
        ".collect()",
        "response accumulation via .extend_from_slice()",
        "response-wide Vec/String construction",
        "Body::from_stream()",
        "SSE construction or decoding",
    ] {
        assert!(
            visitor
                .violations
            ...
            "{expected} escaped streaming architecture analysis: {:?}",
            visitor.violations
        );
    }
}
