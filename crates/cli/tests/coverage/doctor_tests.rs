...
#[tokio::test]
async fn collect_observability_layered_reports_code_driven_source_and_effective_config() {
    let temp = tempfile::tempdir().unwrap();
    let missing = temp.path().join("missing-atif");

    // File/CLI-resolved config: observability present, ATIF disabled.
    let file_value = serde_json::json!({
        "version": 1,
        "components": [{
            "kind": "observability",
            "enabled": true,
            "config": { "version": 1, "atif": { "enabled": false } }
        }]
    });
    // Effective config the code-driven layer produces over the file config: the layer is the higher
    // precedence source and enables ATIF (with a missing output directory), so the effective config
    // has ATIF enabled. `apply_code_driven_plugin_config` returns this merged result; doctor's inner
    // function consumes it directly.
    let effective: PluginConfig = serde_json::from_value(serde_json::json!({
        "version": 1,
        "components": [{
            "kind": "observability",
            "enabled": true,
            "config": { "version": 1, "atif": { "enabled": true, "output_directory": missing } }
        }]
    }))
    .unwrap();

    let checks = collect_observability_layered(Some(&file_value), Some(&effective)).await;

    // The effective source is surfaced as the winning layer.
    let source = checks
        .iter()
        .find(|check| check.name == "Plugin config source")
        .expect("a plugin config source check");
    assert_eq!(source.status, Status::Info);
    assert!(
        source.details.contains("code-driven layer"),
        "{:?}",
        source.details
    );

    // Sub-checks run against the effective (merged) config: ATIF, disabled in the file, is enabled
    // by the layer, so the ATIF directory check appears and warns about the missing directory.
    let atif_check = checks
        .iter()
        .find(|check| check.name == "ATIF dir")
        .expect("ATIF directory check from the effective config");
    assert_eq!(atif_check.status, Status::Warn);
    assert!(!missing.exists());

    // Diagnostics/validation are attributed to the effective config when a layer is active.
    assert!(
        checks
            .iter()
            .any(|check| check.details.contains("effective plugin config")),
        "expected effective-config attribution: {checks:?}"
    );
}

#[tokio::test]
async fn collect_observability_layered_without_file_config_uses_the_layer() {
    // No file/CLI plugin config, only a code-driven layer: the layer alone is the effective config,
    // so doctor reports it as the source and validates it instead of "plugins.toml not configured".
    let effective: PluginConfig = serde_json::from_value(serde_json::json!({
        "version": 1,
    ...
    let checks = collect_observability_layered(None, Some(&effective)).await;

    assert!(
        !checks
            .iter()
            .any(|check| check.details == "plugins.toml not configured"),
        "layer-only config must not report 'not configured': {checks:?}"
    );
    let source = checks
        .iter()
        .find(|check| check.name == "Plugin config source")
        .expect("a plugin config source check");
    assert!(
        source.details.contains("no file/CLI plugin config"),
        "{:?}",
        source.details
    );
    assert!(
        checks
            .iter()
            .any(|check| check.name == "Plugins"
                && check.details.contains("effective plugin config")),
        "expected effective-config validation: {checks:?}"
    );
}
