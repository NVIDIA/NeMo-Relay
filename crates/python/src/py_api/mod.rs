...
use crate::py_types::{
    PyAnnotatedLLMResponse, PyAnthropicMessagesCodec, PyGeminiCodec, PyLLMAttributes, PyLLMHandle,
    PyLLMRequest, PyLlmStream, PyOpenAIChatCodec, PyOpenAIResponsesCodec, PyPropagationContext,
...
fn py_llm_response_codec(
    ...
    response_codec.and_then(|c| -> Option<Arc<dyn LlmResponseCodec>> {
        ...
        if let Ok(builtin) = c.extract::<pyo3::PyRef<'_, PyGeminiCodec>>() {
            return Some(builtin.inner_response_codec.clone());
        }
...
fn py_llm_codec(codec: Option<&Bound<'_, PyAny>>) -> Option<Arc<dyn LlmCodec>> {
    codec.and_then(|codec| -> Option<Arc<dyn LlmCodec>> {
        ...
        if let Ok(builtin) = codec.extract::<pyo3::PyRef<'_, PyGeminiCodec>>() {
            return Some(builtin.inner_codec.clone());
        }
