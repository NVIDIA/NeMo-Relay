...
fn pop_scope(handle: &PyScopeHandle) -> PyResult<()> {
    core_scope_api::pop_scope(&handle.inner.uuid).map_err(to_py_err)
}
...
fn event(
    ...
    core_scope_api::event(name, handle.as_ref().map(|h| &h.inner), data, metadata)
        .map_err(to_py_err)
}
...
fn tool_call_end(
    ...
    let data = opt_py_to_json(data)?;
    let metadata = opt_py_to_json(metadata)?;
    core_tool_api::tool_call_end(&handle.inner, result_json, data, metadata).map_err(to_py_err)
}
...
fn llm_call_end(
    ...
    let data = opt_py_to_json(data)?;
    let metadata = opt_py_to_json(metadata)?;
    core_llm_api::llm_call_end(&handle.inner, response_json, data, metadata, None)
        .map_err(to_py_err)
}
...
fn llm_call_execute<'py>(
    ...
    pyo3_async_runtimes::tokio::future_into_py(py, async move {
        TASK_SCOPE_STACK
            .scope(scope_stack, async move {
                let result = core_llm_api::llm_call_execute(
                    &name,
                    request.inner,
                    default_fn,
                    Some(parent_handle),
...
fn llm_stream_call_execute<'py>(
    ...
    pyo3_async_runtimes::tokio::future_into_py(py, async move {
        TASK_SCOPE_STACK
            .scope(scope_stack, async move {
                let rust_stream = core_llm_api::llm_stream_call_execute(
                    &name,
                    request.inner,
                    default_fn,
                    collector_fn,
