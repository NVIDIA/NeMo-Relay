...
//! Storage abstraction for the nemo-flow-adaptive crate.
//!
...
//! without touching adaptive logic. [`StorageBackendDyn`] adds trie/accumulator
...
/// The storage abstraction for nemo-flow-adaptive persistence.
///
    ...
    fn store_run(&self, record: &RunRecord) -> impl Future<Output = Result<()>> + Send {
        let result = {
            let mut guard = self
                .runs
                .write()
                .map_err(|e| AdaptiveError::Internal(format!("lock poisoned: {e}")));
            match guard {
                Ok(ref mut g) => {
