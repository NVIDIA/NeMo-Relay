...
pub fn pop_scope(handle: &WasmScopeHandle) -> Result<(), JsValue> {
    flow_scope_api::pop_scope(&handle.inner.uuid).map_err(to_js_err)
}
    ...
    match result {
        Ok(ref val) if val.has_type::<js_sys::Promise>() => {
            ...
            let then_cb = Closure::once(move |resolved: JsValue| -> JsValue {
                let _ = flow_scope_api::pop_scope(&then_uuid);
                resolved
            });

            let catch_uuid = scope_uuid;
            let catch_cb = Closure::once(move |rejected: JsValue| -> JsValue {
                let _ = flow_scope_api::pop_scope(&catch_uuid);
        ...
        Ok(val) => {
            ...
            let _ = flow_scope_api::pop_scope(&scope_uuid);
            Ok(val)
        }
        Err(err) => {
...
pub async fn tool_call_execute(
    ...
    let args_json = js_to_json(&args)?;
    let attrs = ToolAttributes::from_bits_truncate(attributes.unwrap_or(0));
    let parent_handle = parent.map(|h| h.inner).unwrap_or_else(task_scope_top);
...
pub async fn llm_stream_call_execute(
    ...
    let rust_stream = TASK_SCOPE_STACK
        .scope(scope_stack, async move {
            flow_llm_api::llm_stream_call_execute(
                name,
                llm_request,
                default_fn,
                wrapped_collector,
