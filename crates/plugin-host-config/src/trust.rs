...
use crate::io::{read_bounded_regular_file, stream_bounded_regular_file};
use crate::policy::EvaluatedDynamicPluginHostPolicy;
...
impl DynamicPluginTrustFailure {
    pub fn display<'a>(&'a self, plugin_id: &'a str) -> DynamicPluginTrustFailureDisplay<'a> {
...
#[doc(hidden)]
pub struct DynamicPluginTrustFailureDisplay<'a> {
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self.failure {
            ...
            DynamicPluginTrustFailure::IntegrityMismatch {
                ...
                f,
                "dynamic plugin '{}' failed integrity verification for {}: the artifact digest does not match the manifest",
                self.plugin_id,
                path.display()
            ),
            ...
            DynamicPluginTrustFailure::InvalidTrustedKey { key: _, error } => write!(
                f,
                "dynamic plugin '{}' has an invalid trusted public key: {}",
...
pub fn evaluate_dynamic_plugin_trust(
    ...
    match evaluate_authenticity(manifest, manifest_ref, &artifact_path, policy) {
...
fn verify_signature(
    ...
    let artifact_bytes = read_bounded_regular_file(artifact_path, "dynamic plugin artifact")
    ...
    for trusted_public_key in trusted_public_keys {
        let public_key_bytes = match parse_ed25519_public_key(trusted_public_key) {
            Ok(bytes) => bytes,
            Err(DynamicPluginTrustFailure::InvalidTrustedKey { error, .. }) => {
...
fn read_signature_bytes(path: &Path) -> TrustResult<Vec<u8>> {
    let raw = read_bounded_regular_file(path, "dynamic plugin signature").map_err(|error| {
...
fn parse_ed25519_public_key(value: &str) -> TrustResult<Vec<u8>> {
    let encoded = value.trim().strip_prefix("ed25519:").ok_or_else(|| {
        DynamicPluginTrustFailure::InvalidTrustedKey {
            key: value.to_owned(),
            error: "unsupported trusted public key format".into(),
        }
    })?;
    base64::engine::general_purpose::STANDARD
        .decode(encoded.trim())
        .map_err(|error| DynamicPluginTrustFailure::InvalidTrustedKey {
            key: value.to_owned(),
            error: format!("invalid ed25519 trusted public key encoding: {error}"),
        })
}
...
fn file_sha256(path: &Path) -> std::result::Result<String, String> {
    ...
    stream_bounded_regular_file(path, "dynamic plugin artifact", |bytes| {
        digest.update(bytes);
    })
