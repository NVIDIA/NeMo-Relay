...
#[test]
fn test_ffi_hosted_plugin_registration_failure_prefers_last_error_message() {
    let _guard = TEST_MUTEX.lock().unwrap();
    reset_globals();
    let _ = nemo_flow_clear_plugin_configuration();

    let plugin_kind = unique_name("ffi_hosted_last_error");
    let plugin_kind_c = cstring(&plugin_kind);
    let config = cstring(
        &json!({
            "version": 1,
    ...
    unsafe {
        assert_eq!(
            nemo_flow_register_plugin(
                plugin_kind_c.as_ptr(),
                None,
                hosted_plugin_register_fail_with_last_error,
                user_data,
                Some(hosted_plugin_free),
            ),
            NemoFlowStatus::Ok
        ...
        assert!(
            read_last_error()
                .unwrap_or_default()
                .contains("hosted register callback set last error explicitly")
        );
