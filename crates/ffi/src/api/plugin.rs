...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn nemo_relay_plugin_context_register_tool_execution_frame_intercept(
    ctx: *mut FfiPluginContext,
    name: *const c_char,
    priority: i32,
    ...
    let wrapped = wrap_tool_exec_frame_intercept_fn(cb, user_data, free_fn);
    match unsafe { &mut *((*ctx).0) }
        .register_tool_execution_frame_intercept(&name, priority, wrapped)
    {
        Ok(()) => NemoRelayStatus::Ok,
        Err(error) => status_from_plugin_error(&error),
    }
