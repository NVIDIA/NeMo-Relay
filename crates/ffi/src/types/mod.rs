...
use nemo_flow::types::scope::{HandleAttributes, ScopeHandle, ScopeType};
#[cfg(test)]
...
pub unsafe extern "C" fn nemo_flow_event_attributes(ptr: *const FfiEvent) -> u32 {
    ...
    match unsafe { &*ptr }.0.attributes() {
        Some(HandleAttributes::Scope(attributes)) => attributes.bits(),
        Some(HandleAttributes::Tool(attributes)) => attributes.bits(),
        Some(HandleAttributes::Llm(attributes)) => attributes.bits(),
        None => 0,
    }
}
...
pub unsafe extern "C" fn nemo_flow_event_annotated_request(ptr: *const FfiEvent) -> *mut c_char {
    ...
    match &unsafe { &*ptr }.0 {
        Event::LLMStart(inner) => match &inner.annotated_request {
            Some(a) => {
...
pub unsafe extern "C" fn nemo_flow_event_annotated_response(ptr: *const FfiEvent) -> *mut c_char {
    ...
    match &unsafe { &*ptr }.0 {
        Event::LLMEnd(inner) => match &inner.annotated_response {
            Some(a) => {
                let value = serde_json::to_value(a.as_ref()).unwrap_or_default();
                json_to_c_string(&value)
            }
