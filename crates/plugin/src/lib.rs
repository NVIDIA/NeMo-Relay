...
struct PluginState<P> {
    host: NemoRelayNativeHostApiV1,
    registration_host: HostApiStorage,
    plugin: Mutex<P>,
}
...
impl HostApiStorage {
    unsafe fn copy_from(host: &NemoRelayNativeHostApiV1) -> Self {
        ...
        {
            return Self::V3(unsafe { *(host as *const _ as *const NemoRelayNativeHostApiV3) });
        }
        Self::V1(*host)
    }
...
unsafe extern "C" fn register_trampoline<P: NativePlugin>(
    ...
    let result = catch_unwind(AssertUnwindSafe(|| {
        ...
        let mut ctx = unsafe { PluginContext::from_raw(state.registration_host.v1(), ctx) };
    ...
    F: FnOnce() -> P,
{
    ...
    let state = Box::new(PluginState {
        host: *host_ref,
        registration_host: unsafe { HostApiStorage::copy_from(host_ref) },
