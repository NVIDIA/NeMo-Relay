...
| Native ABI v7 | Keeps C-compatible host and plugin tables behind the safe Rust interface. Relay 0.10 adds directional codec context for LLM execution callbacks and requires native plugins to rebuild against that layout. |
...
Because this changes callback layouts, the 0.10 host rejects v2-v6 tables.
Rebuild every plugin with the 0.10 v7 SDK and set
