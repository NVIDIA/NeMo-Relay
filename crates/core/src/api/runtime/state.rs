...
use crate::api::runtime::callbacks::{
    ...
    LlmStreamExecutionNextFn, LlmStreamExecutionRegistryRefs, ToolConditionalFn,
...
struct ToolContinuationInvocationGuard {
    active: Arc<AtomicBool>,
}

impl ToolContinuationInvocationGuard {
    fn new() -> Self {
        Self {
...
impl Drop for ToolContinuationInvocationGuard {
    fn drop(&mut self) {
        ...
        for (callable, _) in matching.into_iter().rev() {
            ...
            next = match callable {
                ToolExecutionCallable::Legacy(callable) => Arc::new(move |args| {
                    ...
                    Box::pin(async move {
                        let mut legacy_outcome = callable(&current_name, args, raw_next).await?;
                ...
                ToolExecutionCallable::Frame(callable) => Arc::new(move |args| {
                    let callable = callable.clone();
                    let current_name = current_name.clone();
                    let invocation = ToolContinuationInvocationGuard::new();
                    let continuation_active = invocation.active_flag();
                    let next_sequence = Arc::new(AtomicUsize::new(0));
                    let downstream_marks = Arc::new(Mutex::new(Vec::new()));
                    let frame_next: ToolExecutionFrameNextFn = {
                        let current_next = current_next.clone();
                        let continuation_active = continuation_active.clone();
                        let next_sequence = next_sequence.clone();
                        let downstream_marks = downstream_marks.clone();
                        Arc::new(move |args| {
                            let was_active = continuation_active.load(Ordering::Acquire);
                            let sequence = next_sequence.fetch_add(1, Ordering::Relaxed);
                            let current_next = current_next.clone();
                            let continuation_active = continuation_active.clone();
                            let downstream_marks = downstream_marks.clone();
                            Box::pin(async move {
                                if !was_active || !continuation_active.load(Ordering::Acquire) {
                                    return Err(crate::error::FlowError::Internal(
                                        "tool execution continuation is no longer active".into(),
                                    ));
                                }
                                let outcome = current_next(args).await?;
                                downstream_marks
