    ...
    let (callback, _shutdown, mut cancel_rx) = fake_callback_service_with_handlers(
        {
            ...
            move |request| {
                ...
                let started_tx = Arc::clone(&started_tx);
                Box::pin(async move {
                    let continuation = continuation_for(&host_state, &request.continuation_id);
                    let Continuation::Tool { next, .. } = continuation else {
                        panic!("expected a tool continuation");
                    };
                    if let Some(started) = started_tx.lock().unwrap().take() {
                        let _ = started.send(());
                    }
                    let response = next(json!({"input": "slow"}))
                        .await
                        .expect("slow downstream tool must complete");
                    InvokeResponse {
                        result: Some(InvokeResult::ToolExecution(ToolExecutionInterceptResult {
                            outcome: Some(ProtoToolExecutionInterceptOutcome {
                                result: Some(JsonValue {
                                    json: serde_json::to_vec(&response.result).unwrap(),
                                }),
                                annotation: None,
                                pending_marks: None,
                            }),
    ...
    let result = tokio::spawn(async move {
        callback_task
            .invoke_tool_execution(
                ...
                Arc::new(|value| {
                    Box::pin(async move {
                        tokio::time::sleep(WORKER_RPC_TIMEOUT + Duration::from_secs(1)).await;
                        Ok(ToolExecutionResult::new(json!({"slow": value})))
                    })
                }),
            )
            .await
    });
    started_rx.await.expect("worker tool callback must start");
    tokio::time::advance(WORKER_RPC_TIMEOUT + Duration::from_secs(2)).await;

    assert_eq!(
        result.await.unwrap().unwrap().result,
        json!({"slow": {"input": "slow"}})
    );
    assert_no_worker_cancellation(&mut cancel_rx, "slow downstream tool execution");
}
...
async fn continuation_bearing_worker_callback_allows_slow_next() {
            ...
            move |request| {
                ...
                Box::pin(async move {
                    ...
                    let Continuation::Llm { next, .. } = continuation else {
                        panic!("expected an LLM continuation");
                    };
                    if let Some(started) = started_tx.lock().unwrap().take() {
                        let _ = started.send(());
                    }
                    tokio::time::sleep(WORKER_RPC_TIMEOUT + Duration::from_secs(1)).await;
                    let response = next(valid_llm_request())
                        .await
                        .expect("slow downstream provider must complete");
                    InvokeResponse {
                        result: Some(InvokeResult::Json(JsonResult {
                            value: Some(json_envelope(JSON_SCHEMA, &response).unwrap()),
                            error: None,
                        })),
                    }
                })
    ...
    *host_state.lock().unwrap() = Some(callback.host_state.clone());
    ...
    started_rx.await.expect("worker callback must start");
    tokio::time::advance(WORKER_RPC_TIMEOUT + Duration::from_secs(2)).await;

    assert_eq!(result.await.unwrap().unwrap(), json!({"slow": "completed"}));
    assert_no_worker_cancellation(&mut cancel_rx, "slow downstream execution");
}

#[tokio::test(start_paused = true)]
async fn continuation_bearing_worker_stream_allows_slow_next() {
    enable_operational_logs();
    let host_state = shared_worker_host_state();
    let (started_tx, started_rx) = oneshot::channel();
    let started_tx = Arc::new(Mutex::new(Some(started_tx)));
    let (callback, _shutdown, mut cancel_rx) = fake_callback_service_with_async_stream_handler(
        |_| {
            Box::pin(async {
                InvokeResponse {
                    result: Some(InvokeResult::Empty(EmptyResult {})),
            ...
            move |request| {
                ...
                Box::pin(async move {
                    ...
                    let Continuation::LlmStream { next, .. } = continuation else {
                        panic!("expected an LLM stream continuation");
                    };
                    if let Some(started) = started_tx.lock().unwrap().take() {
                        let _ = started.send(());
                    }
                    tokio::time::sleep(WORKER_RPC_TIMEOUT + Duration::from_secs(1)).await;
                    let stream = next(valid_llm_request())
                        .await
                        .expect("slow downstream stream must open");
                    Box::pin(stream.map(|item| {
                        item.map(|value| StreamChunk {
                            item: Some(StreamItem::Value(
                                json_envelope(JSON_SCHEMA, &value)
                                    .expect("stream value must encode"),
                            )),
                        })
    ...
    *host_state.lock().unwrap() = Some(callback.host_state.clone());
    ...
    started_rx.await.expect("worker stream callback must start");
    tokio::time::advance(WORKER_RPC_TIMEOUT + Duration::from_secs(2)).await;

    let mut stream = result.await.unwrap().expect("slow worker stream must open");
    assert_eq!(
        stream.next().await.unwrap().unwrap(),
        json!({"slow_stream": "completed"})
    );
    assert!(stream.next().await.is_none());
    assert_no_worker_cancellation(&mut cancel_rx, "slow downstream stream setup");
}
...
fn continuation_for(state: &SharedWorkerHostState, continuation_id: &str) -> Continuation {
    state
        .lock()
        .unwrap()
        .as_ref()
        ...
        .expect("continuation must remain active")
}

fn assert_no_worker_cancellation(
    cancel_rx: &mut mpsc::UnboundedReceiver<CancelInvocationRequest>,
    operation: &str,
) {
    assert!(
        matches!(
        ...
        "{operation} must not trigger worker cancellation"
    );
}
...
async fn fake_callback_service_with_async_stream_handler(
    invoke: impl Fn(InvokeRequest) -> FakeInvokeFuture + Send + Sync + 'static,
    ...
    let (client, shutdown_tx, cancel_rx, _register_calls) =
        fake_worker_client_with_async_handlers(invoke, invoke_stream).await;
    let (callback, shutdown_tx) = callback_for_client(client, shutdown_tx);
    (callback, shutdown_tx, cancel_rx)
}
