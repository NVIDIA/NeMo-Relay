...
#[tokio::test]
async fn test_legacy_continuation_retains_existing_post_callback_behavior() {
    let _lock = TEST_MUTEX.lock().unwrap();
    reset_global();
    setup_isolated_thread();
    ...
    let retained = retained.lock().unwrap().take().unwrap();
    let late_result = retained(json!({"late": true})).await.unwrap();
    assert_eq!(late_result, json!({"late": true}));
    assert!(producer_called.load(Ordering::SeqCst));
