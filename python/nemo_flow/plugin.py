...
class ConfigDiagnostic(_ConfigDiagnosticRequired, total=False):
    """One plugin-host validation diagnostic."""
...
class ConfigReport(TypedDict):
    """Validation or activation report for a plugin host config."""
...
class ConfigPolicy:
    """Policy for unsupported plugin-host configuration.
...
def validate(config: PluginConfig | JsonObject) -> ConfigReport:
    """Validate a plugin-host configuration without changing runtime state.
...
async def initialize(config: PluginConfig | JsonObject) -> ConfigReport:
    """Validate and activate a plugin-host configuration.
...
def report() -> ConfigReport | None:
    """Return the last successful plugin-host report.
