...
| pi | Partial | Partial | No | Proof of concept. A Relay-authored pi extension forwards tool and turn activity and gates tool calls; pi's model traffic does not traverse the gateway, so there are no LLM spans. |
