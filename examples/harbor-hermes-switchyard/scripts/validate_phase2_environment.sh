...
if [[ -z "$env_file" || "$env_file" != /* || ! -f "$env_file" ]]; then
  echo "usage: $0 /absolute/phase2-run.env" >&2
  exit 2
fi
...
required_values=(
  EXAMPLE_ROOT PHASE2_RUN_ID PHASE2_RUN_ROOT PHASE2_ADMISSION_ROOT
  HARBOR_BIN EVAL_PYTHON TBENCH_DATASET_PATH SWITCHYARD_BUNDLE RELAY_WHEEL
  RELAY_ARCHITECTURE PLUGIN_CONFIG_TEMPLATE PHASE2_SMOKE_EVIDENCE
  PHASE2_OFFLINE_EVIDENCE PHOENIX_BASE_URL PHOENIX_PROJECT EVAL_COHORT
...
for name in EXAMPLE_ROOT PHASE2_RUN_ROOT PHASE2_ADMISSION_ROOT HARBOR_BIN EVAL_PYTHON \
  TBENCH_DATASET_PATH SWITCHYARD_BUNDLE RELAY_WHEEL PLUGIN_CONFIG_TEMPLATE \
  PHASE2_SMOKE_EVIDENCE PHASE2_OFFLINE_EVIDENCE; do
  if [[ "${!name}" != /* ]]; then
    echo "Phase 2 path must be absolute: $name" >&2
    exit 2
  fi
done
for name in TBENCH_SAMPLE_COUNT TBENCH_CONCURRENCY TBENCH_PARALLEL_MAX_MEMORY_GB \
